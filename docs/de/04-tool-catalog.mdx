...
description: >-
  Vollständiger Katalog von über 50 auf der Traffic-Generator-VM installierten
  Sicherheitstools, organisiert nach Kategorie mit Installationsmethode,
  Beispielbefehlen und Traffic-Suite-Zuordnungen
sidebar:
  order: 4
i18n:
...
Alle unten aufgeführten Tools sind während der Cloud-Init-Bereitstellung vorinstalliert. Nach dem Deployment ist keine manuelle Installation erforderlich.
...
:::note
ZAP und Metasploit fügen dem Speicherbedarf etwa 1,5 GiB hinzu und verlängern die Bereitstellungszeit um 5-10 Minuten. Verwenden Sie die Vollversion nur, wenn Sie diese Tools speziell für eine Demo benötigen.
:::
...
|---|---|
| apt | Installiert aus Ubuntu 24.04 APT-Repositories über `apt-get install` |
| ghlatest | Neuestes Binary von GitHub Releases heruntergeladen mit `ghlatest.sh` und `install-release.sh` |
...
| go install | Aus Quellcode kompiliert mit `go install` und installiert nach `/usr/local/bin` |
...
| installer script | Vom Anbieter bereitgestelltes Installationsskript, ausgeführt während Cloud-Init |
