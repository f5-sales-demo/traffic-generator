...
title: Tool-Katalog
description: >-
  Vollständiger Katalog von über 50 Sicherheitstools, die auf der
...
Alle unten aufgeführten Tools sind während der Cloud-Init-Provisionierung vorinstalliert. Nach dem Deployment ist keine manuelle Installation erforderlich.

## Testen von Webanwendungen

| Tool | Installationsmethode | Beispielbefehl | Verwendet von Suite |
|---|---|---|---|
...
| puppeteer-extra-plugin-stealth | npm | Wird als Puppeteer-Plugin für getarntes Browsing verwendet | bot-simulation |

Playwright installiert seine eigene Chromium-Binary über `playwright install chromium`. Puppeteer verwendet nach Möglichkeit dasselbe System-Chromium.
...
| Tool | Installationsmethode | Speicherort | Verwendet von Suite |
|---|---|---|---|
...
SecLists stellt Wortlisten bereit, die von mehreren Tools verwendet werden, darunter ffuf, gobuster, feroxbuster, dirb und hydra. PayloadsAllTheThings bietet kuratierte Multi-Encoding-Angriffs-Payloads, organisiert nach Angriffsklasse (XSS, SQLi, XXE) mit Hex-, Oktal-, Unicode-, HTML-Entity-, doppelter URL-Kodierung und JSFuck-Varianten.

## Nur im vollständigen Tier

Diese Tools werden nur installiert, wenn `tool_tier = "full"`:
...
:::note
ZAP und Metasploit fügen dem Speicherbedarf ca. 1,5 GiB hinzu und verlängern die Provisionierungszeit um 5-10 Minuten. Verwenden Sie das vollständige Tier nur, wenn Sie diese Tools speziell für eine Demo benötigen.
:::
...
| ghlatest | Neueste Binary von GitHub Releases heruntergeladen mittels `ghlatest.sh` und `install-release.sh` |
| uv | Installiert als Python-Tool mit dem `uv`-Paketmanager |
| pip | Installiert als Python-Paket mittels `pip install` in das System-Python |
| npm | Installiert als globales Node.js-Paket mittels `npm install -g` |
| go install | Aus Quellcode kompiliert mittels `go install` und installiert nach `/usr/local/bin` |
| git clone | Repository geklont in das `/opt/`-Verzeichnis |
