...
Diese Komponente stellt eine automatisierte Traffic-Generierungsplattform bereit, die Angriffsverkehr, Reconnaissance-Scans, Bot-Simulation und API-Missbrauch gegen einen F5 Distributed Cloud HTTP Load Balancer erzeugt. Sie ist der "Angreifer" in einer typischen Demo-Architektur – die Quelle von bösartigem und verdächtigem Traffic, den die Sicherheitsfunktionen von F5 XC erkennen und blockieren sollen.
...
- **19 organisierten Traffic-Suites** mit nummerierten Skripten, die in Reihenfolge ausgeführt werden
...
Fügt OWASP ZAP und Metasploit Framework zusätzlich zur Standard-Stufe hinzu. Die Bereitstellung dauert ungefähr 25 Minuten. Diese Tools sind groß (ZAP ~500 MiB, Metasploit ~1 GiB) und werden nur für fortgeschrittene Schwachstellen-Scanning-Demos benötigt.

Aktuelle VM-Kosten finden Sie im Azure-Preisrechner. Die standardmäßige Standard_F16s_v2 ist eine compute-optimierte Instanz, die für anhaltende Traffic-Generierung geeignet ist.
...
Diese Komponente integriert sich mit zwei anderen Demo-Komponenten:
...
- **CSD-Demo** – Die Client-Side-Defense-Demoanwendung auf dem Origin Server. Die `javascript-exploits`-Traffic-Suite generiert Magecart-artige Skript-Injection-Payloads, die F5 XC Client-Side Defense erkennt. Dies validiert die CSD-Phase-2-Funktionalität.
