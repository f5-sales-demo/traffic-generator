...
`runner.sh` ist der Suite-Orchestrator, der alle nummerierten Skripte in einem Suite-Verzeichnis der Reihe nach ausführt, die Ausgabe erfasst und Ergebnis-Metadaten aufzeichnet. Er wird auf der VM unter `/opt/traffic-generator/suites/runner.sh` installiert.
...
Jede Variable in `config.env` kann überschrieben werden, indem sie vor der Ausführung der Suite exportiert wird:
...
Das `--dry-run`-Flag gibt jedes Skript aus, das ausgeführt würde, ohne es tatsächlich auszuführen:
...
Die stdout- und stderr-Ausgabe jedes Skripts wird in einer `.log`-Datei erfasst, die nach dem Skript benannt ist.

### meta.json Format

Die Datei `meta.json` zeichnet die Ausführungs-Metadaten der Suite auf:
...
<Aside type="tip">
Ein Exit-Code ungleich Null eines Skripts bedeutet nicht unbedingt, dass die Suite fehlgeschlagen ist. Viele Sicherheitstools geben einen Exit-Code ungleich Null zurück, wenn sie Schwachstellen finden, was das erwartete Verhalten bei der Generierung von Angriffs-Traffic ist.
</Aside>
...
Skripte akzeptieren den Ziel-FQDN als erstes Positionsargument. Sie lesen `config.env` nicht direkt -- das macht nur der Runner. Bei eigenständiger Ausführung müssen Sie den FQDN explizit übergeben.

## Alle Suites nacheinander ausführen

Um alle Suites hintereinander auszuführen:
...
Dies erzeugt umfassenden Traffic über alle F5 XC-Sicherheitsfunktionen hinweg. Erwarten Sie, dass der vollständige Durchlauf je nach Antwortzeiten des Ziels 45-80 Minuten dauert.
...
Verwenden Sie bei lang laufenden Suites `nohup` oder `tmux`, um zu verhindern, dass eine SSH-Verbindungsunterbrechung den Prozess beendet:
...
3. **Ergebnisverzeichnis erstellen** -- Erstellt `/opt/traffic-generator/results/<timestamp>-<suite>/` und schreibt eine initiale `meta.json` mit `status: running`.
