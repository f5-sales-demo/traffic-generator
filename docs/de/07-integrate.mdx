...
description: >-
  ...
  Zielkonfiguration, Vergleich zwischen direktem und LB-Traffic sowie
  Multi-Komponenten-Deployment
sidebar:
  order: 7
i18n:
...
Der Traffic-Generator ist eine Komponente in einer mehrschichtigen Demo-Umgebung. Die vollständige Architektur bei Deployment aller Komponenten:
...
Jede Komponente wird unabhängig deployed und über Terraform konfiguriert. Der Traffic-Generator zielt auf den F5 XC Load Balancer FQDN, nicht direkt auf den Origin-Server.
...
1. Deployen Sie zuerst den **Origin-Server** -- er stellt die Backend-Anwendungen bereit
2. Konfigurieren Sie den **F5 XC HTTP Load Balancer** mit dem Origin-Server als Origin Pool
3. Fügen Sie **WAF-, Bot-Defense-, API-Security- und CSD-Richtlinien** dem Load Balancer hinzu
4. Deployen Sie den **Traffic-Generator** mit `target_fqdn` auf die F5 XC LB-Domain gesetzt

### Zielkonfiguration

Die `config.env` des Traffic-Generators verbindet ihn mit der übrigen Architektur:
...
2. F5 XC CSD injiziert sein Monitoring-JavaScript in die Seite
...
So verwenden Sie die javascript-exploits-Suite:

```bash
# Stellen Sie sicher, dass CSD auf dem F5 XC HTTP LB für den Pfad /csd-demo/ aktiviert ist
# Führen Sie dann die Suite aus
/opt/traffic-generator/suites/runner.sh javascript-exploits
```

## CDN-Simulator-Integration

Wenn der CDN-Simulator deployed ist, fügt die Architektur eine Caching-Schicht hinzu:

```
Traffic Generator -> CDN Simulator -> F5 XC HTTP LB -> Origin Server
```

Der CDN-Simulator sitzt vor dem F5 XC Load Balancer, cached Antworten und fügt CDN-ähnliche Header hinzu. Um Traffic durch den CDN zu leiten:
...
Dies ist nützlich, um zu demonstrieren, wie F5 XC Traffic behandelt, der über einen CDN ankommt, einschließlich:

- Identifizierung der echten Client-IP hinter CDN-Proxy-Headern
...
Erwartet: WAF blockiert SQL-Injection-, XSS- und Command-Injection-Payloads. Das Security-Events-Dashboard zeigt blockierte Anfragen mit Verletzungsdetails.
...
1. Führen Sie `web-app-attacks` direkt gegen den Origin aus -- zeigen Sie, dass Angriffe erfolgreich sind
2. Führen Sie `web-app-attacks` über F5 XC aus -- zeigen Sie, dass Angriffe blockiert werden
...
## Multi-Komponenten-Terraform-Deployment

Beim Deployment der vollständigen Lab-Umgebung verwenden Sie separate Terraform-Workspaces oder -Verzeichnisse für jede Komponente:

```bash
# 1. Origin-Server deployen
cd origin-server
...
# 3. Traffic-Generator mit Ziel F5 XC LB deployen
