...
`runner.sh` هو منسق المجموعة الذي ينفذ جميع السكربتات المرقمة في مجلد المجموعة بالترتيب، ويلتقط المخرجات، ويسجل بيانات النتائج الوصفية. يتم تثبيته في المسار `/opt/traffic-generator/suites/runner.sh` على الجهاز الافتراضي.
...
يقرأ المنسق التكوين من `/opt/traffic-generator/config.env`:
...
يتم كتابة هذا الملف تلقائيًا أثناء توفير Terraform من المتغيرين `target_fqdn` و `target_origin_ip`. لتغيير الهدف بعد النشر، قم بتحرير هذا الملف مباشرة على الجهاز الافتراضي.
...
يطبع العلم `--dry-run` كل سكربت سيتم تنفيذه دون تشغيله فعليًا:
...
ينشئ كل تشغيل لمجموعة مجلد نتائج مختومًا بالتاريخ والوقت:
...
|---|---|
| `suite` | اسم المجموعة |
| `target` | اسم النطاق الكامل (FQDN) المستخدم للتشغيل |
...
| `passed` | عدد السكربتات التي خرجت برمز 0 |
...
<Aside type="tip">
لا يعني بالضرورة رمز خروج غير صفري من سكربت أن المجموعة فشلت. تخرج العديد من أدوات الأمان برمز غير صفري عندما تجد ثغرات أمنية، وهو السلوك المتوقع أثناء توليد حركة مرور الهجمات.
</Aside>

## تشغيل السكربتات الفردية

يمكن تنفيذ كل سكربت بشكل مستقل بدون المنسق:
...
تقبل السكربتات اسم النطاق الكامل (FQDN) للهدف كأول وسيط موضعي. لا تقرأ `config.env` مباشرة -- فقط المنسق يقوم بذلك. عند التشغيل المستقل، يجب تمرير FQDN بشكل صريح.
...
1. **تحميل التكوين** -- يقرأ المنسق `config.env` (أو تجاوز `CONFIG_FILE`)، ثم يتحقق من وجود `TARGET_FQDN` في البيئة.

2. **التحقق من المجموعة** -- يؤكد وجود مجلد المجموعة المسمى ضمن مجلد المجموعات. يعرض المجموعات المتاحة إذا لم يتم العثور عليه.

3. **إنشاء مجلد النتائج** -- ينشئ `/opt/traffic-generator/results/<timestamp>-<suite>/` ويكتب `meta.json` الأولي بحالة `status: running`.

4. **تنفيذ السكربتات** -- يتكرر عبر الملفات المطابقة لنمط `[0-9]*` في مجلد المجموعة، مرتبة حسب الاسم. يتخطى الملفات غير القابلة للتنفيذ. يمرر `TARGET_FQDN` كأول وسيط. يلتقط المخرجات في `<script-name>.log`.

5. **تسجيل النتائج** -- يحدّث `meta.json` بالطابع الزمني للاكتمال وأعداد النجاح/الفشل/التخطي.
