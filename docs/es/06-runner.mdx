...
La bandera `--dry-run` muestra cada script que se ejecutaría sin ejecutarlo realmente:
...
Use dry-run para verificar la estructura de la suite después del despliegue o antes de ejecutar una suite contra un nuevo destino.
...
La salida stdout y stderr de cada script se captura en un archivo `.log` con el nombre del script.
...
| `target` | FQDN del destino utilizado para la ejecución |
...
| `passed` | Número de scripts que terminaron con código de salida 0 |
| `failed` | Número de scripts que terminaron con código de salida distinto de cero |
| `skipped` | Número de scripts omitidos (no ejecutables o en dry-run) |

<Aside type="tip">
Un código de salida distinto de cero de un script no significa necesariamente que la suite haya fallado. Muchas herramientas de seguridad terminan con código distinto de cero cuando encuentran vulnerabilidades, lo cual es el comportamiento esperado durante la generación de tráfico de ataque.
</Aside>
...
Los scripts aceptan el FQDN del destino como primer argumento posicional. No leen `config.env` directamente -- solo el runner lo hace. Al ejecutarlos de forma independiente, debe pasar el FQDN explícitamente.

## Ejecución de todas las suites en secuencia

Para ejecutar todas las suites consecutivamente:
...
Esto genera tráfico exhaustivo que abarca todas las funcionalidades de seguridad de F5 XC. Espere que la ejecución completa tome entre 45 y 80 minutos dependiendo de los tiempos de respuesta del destino.
...
Para suites de larga duración, use `nohup` o `tmux` para evitar que una desconexión SSH termine el proceso:
