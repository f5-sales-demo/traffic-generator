...
La bandera `--dry-run` imprime cada script que se ejecutaría sin ejecutarlo:
...
La salida estándar (stdout) y la salida de error (stderr) de cada script se capturan en un archivo `.log` con el nombre del script.
...
| `started` | Marca de tiempo UTC de inicio de la suite |
| `completed` | Marca de tiempo UTC de finalización de la suite |
| `status` | `running` durante la ejecución, `completed` cuando finaliza |
...
Los scripts aceptan el FQDN del objetivo como primer argumento posicional. No leen `config.env` directamente -- solo el runner lo hace. Al ejecutarlos de forma independiente, debe pasar el FQDN explícitamente.
...
2. **Validar suite** -- Confirma que el directorio de suite nombrado existe bajo el directorio de suites. Lista las suites disponibles si no se encuentra.
