...
description: >-
  ...
  comandos de ejemplo y mapeos de suites de tráfico
sidebar:
  order: 4
i18n:
...
Todas las herramientas listadas a continuación están preinstaladas durante el aprovisionamiento con cloud-init. No se necesita instalación manual después del despliegue.
...
GoTestWAF (de Wallarm) es un framework estándar de la industria para evaluación de WAF que multiplica payloads a través de múltiples codificadores (URL, Base64, JSUnicode) y zonas de ubicación (body, header, parámetro URL, cookie). waf-bypass (de Nemesida) envía payloads con codificación Base64, HTML-ENTITY y UTF-16 a través de las zonas ARGS, BODY, COOKIE y HEADER.
...
| puppeteer-extra-plugin-stealth | npm | Utilizado como plugin de Puppeteer para navegación sigilosa | bot-simulation |
...
## Bibliotecas de Seguridad en Python
...
SecLists proporciona listas de palabras utilizadas por múltiples herramientas incluyendo ffuf, gobuster, feroxbuster, dirb e hydra. PayloadsAllTheThings proporciona payloads de ataque curados con múltiples codificaciones, organizados por clase de ataque (XSS, SQLi, XXE) con variantes en hexadecimal, octal, Unicode, entidades HTML, doble codificación URL y JSFuck.
...
:::note
ZAP y Metasploit añaden aproximadamente 1,5 GiB de espacio en disco y entre 5 y 10 minutos al tiempo de aprovisionamiento. Utilice el nivel completo únicamente si necesita específicamente estas herramientas para una demostración.
:::
...
| ghlatest | Último binario descargado desde GitHub Releases usando `ghlatest.sh` e `install-release.sh` |
| uv | Instalado como herramienta Python usando el gestor de paquetes `uv` |
| pip | Instalado como paquete Python usando `pip install` en el Python del sistema |
| npm | Instalado como paquete global de Node.js usando `npm install -g` |
| go install | Compilado desde el código fuente usando `go install` e instalado en `/usr/local/bin` |
| git clone | Repositorio clonado en el directorio `/opt/` |
| git clone + pip | Repositorio clonado y dependencias de Python instaladas |
