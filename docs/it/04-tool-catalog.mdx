...
GoTestWAF (di Wallarm) è un framework di valutazione WAF standard del settore che moltiplica i payload attraverso diversi encoder (URL, Base64, JSUnicode) e zone di posizionamento (body, header, parametro URL, cookie). waf-bypass (di Nemesida) invia payload con codifica Base64, HTML-ENTITY e UTF-16 attraverso le zone ARGS, BODY, COOKIE e HEADER.
...
| puppeteer-extra-plugin-stealth | npm | Utilizzato come plugin di Puppeteer per la navigazione stealth | bot-simulation |
...
SecLists fornisce wordlist utilizzate da più strumenti tra cui ffuf, gobuster, feroxbuster, dirb e hydra. PayloadsAllTheThings fornisce payload di attacco curati con codifiche multiple, organizzati per classe di attacco (XSS, SQLi, XXE) con varianti hex, ottali, Unicode, entità HTML, doppia codifica URL e JSFuck.
...
:::note
ZAP e Metasploit aggiungono circa 1,5 GiB all'occupazione disco e 5-10 minuti al tempo di provisioning. Utilizzate il livello completo solo se avete specificamente bisogno di questi strumenti per una demo.
:::
...
| ghlatest | Ultimo binario scaricato da GitHub Releases utilizzando `ghlatest.sh` e `install-release.sh` |
