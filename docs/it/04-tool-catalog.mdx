...
description: >-
  ...
  comandi di esempio e mappatura delle suite di traffico
sidebar:
  order: 4
i18n:
...
Tutti gli strumenti elencati di seguito sono pre-installati durante il provisioning cloud-init. Non è necessaria alcuna installazione manuale dopo il deployment.

## Test di Applicazioni Web

| Strumento | Metodo di Installazione | Comando di Esempio | Usato dalla Suite |
|---|---|---|---|
...
GoTestWAF (di Wallarm) è un framework di valutazione WAF standard di settore che moltiplica i payload attraverso diversi encoder (URL, Base64, JSUnicode) e zone di posizionamento (body, header, parametro URL, cookie). waf-bypass (di Nemesida) invia payload con codifica Base64, HTML-ENTITY e UTF-16 attraverso le zone ARGS, BODY, COOKIE e HEADER.
...
| puppeteer-extra-plugin-stealth | npm | Utilizzato come plugin Puppeteer per la navigazione stealth | bot-simulation |

Playwright installa il proprio binario Chromium tramite `playwright install chromium`. Puppeteer utilizza lo stesso Chromium di sistema quando possibile.
...
| Strumento | Metodo di Installazione | Posizione | Usato dalla Suite |
|---|---|---|---|
...
SecLists fornisce wordlist utilizzate da diversi strumenti tra cui ffuf, gobuster, feroxbuster, dirb e hydra. PayloadsAllTheThings fornisce payload di attacco multi-codifica curati, organizzati per classe di attacco (XSS, SQLi, XXE) con varianti hex, ottale, Unicode, entità HTML, doppia codifica URL e JSFuck.
...
:::note
ZAP e Metasploit aggiungono circa 1,5 GiB all'occupazione su disco e 5-10 minuti al tempo di provisioning. Utilizzate il livello full solo se avete specificamente bisogno di questi strumenti per una demo.
:::

## Riferimento dei Metodi di Installazione

| Metodo | Descrizione |
|---|---|
| apt | Installato dai repository APT di Ubuntu 24.04 tramite `apt-get install` |
| ghlatest | Ultimo binario scaricato dalle GitHub Releases utilizzando `ghlatest.sh` e `install-release.sh` |
| uv | Installato come strumento Python utilizzando il gestore di pacchetti `uv` |
| pip | Installato come pacchetto Python utilizzando `pip install` nel Python di sistema |
| npm | Installato come pacchetto globale Node.js utilizzando `npm install -g` |
| go install | Compilato dal sorgente utilizzando `go install` e installato in `/usr/local/bin` |
| git clone | Repository clonato nella directory `/opt/` |
| git clone + pip | Repository clonato e dipendenze Python installate |
| installer script | Script di installazione fornito dal produttore eseguito durante il cloud-init |
