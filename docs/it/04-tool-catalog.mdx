...
GoTestWAF (di Wallarm) è un framework di valutazione WAF standard del settore che moltiplica i payload attraverso diversi encoder (URL, Base64, JSUnicode) e zone di inserimento (body, header, parametro URL, cookie). waf-bypass (di Nemesida) invia payload con codifica Base64, HTML-ENTITY e UTF-16 attraverso le zone ARGS, BODY, COOKIE e HEADER.
...
| puppeteer-extra-plugin-stealth | npm | Utilizzato come plugin Puppeteer per navigazione stealth | bot-simulation |
...
## Enumerazione Sottodomini e DNS
...
SecLists fornisce wordlist utilizzate da diversi strumenti tra cui ffuf, gobuster, feroxbuster, dirb e hydra. PayloadsAllTheThings fornisce payload di attacco curati con codifiche multiple, organizzati per classe di attacco (XSS, SQLi, XXE) con varianti hex, ottale, Unicode, entità HTML, doppia codifica URL e JSFuck.
...
:::note
ZAP e Metasploit aggiungono circa 1,5 GiB all'occupazione su disco e 5-10 minuti al tempo di provisioning. Utilizzare il livello full solo se si necessita specificamente di questi strumenti per una demo.
:::

## Riferimento Metodi di Installazione
