...
description: >-
  ...
  HTTP load balancer, demo CSD e simulatore CDN: diagrammi di architettura,
...
Ogni componente è distribuito e configurato indipendentemente tramite Terraform. Il generatore di traffico punta al FQDN del load balancer F5 XC, non direttamente al server di origine.
...
| ssl-scanning | F5 XC LB (non direttamente l'origine) | N/D |
...
2. Configurare il **load balancer HTTP F5 XC** con l'origin server come origin pool
3. Associare le **policy WAF, Bot Defense, API Security e CSD** al load balancer
...
La suite `javascript-exploits` è specificamente progettata per la demo Client-Side Defense sull'origin server. Questa suite valida la funzionalità CSD Fase 2:
...
Il Simulatore CDN si posiziona davanti al load balancer F5 XC, effettuando il caching delle risposte e aggiungendo header tipici delle CDN. Per indirizzare il traffico attraverso la CDN:
...
- Identificare il vero IP del client dietro gli header proxy della CDN
- Applicare regole WAF a richieste che potrebbero essere state modificate dalla CDN
- Classificazione Bot Defense quando la CDN modifica le impronte digitali del browser
...
Risultato atteso: tutti i payload raggiungono le applicazioni di origine senza filtri. Juice Shop e DVWA elaborano i payload di attacco. Questo dimostra cosa accade senza la protezione F5 XC.
