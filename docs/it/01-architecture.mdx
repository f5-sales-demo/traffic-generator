...
|---|---|---|
| Test di applicazioni web | nikto, sqlmap, nuclei, dalfox, ffuf, gobuster, feroxbuster, dirb, whatweb | Generazione di payload di attacco WAF |
...
| Test delle credenziali | hydra, medusa, ncrack | Simulazione di attacchi all'autenticazione |
| Test di evasione WAF | gotestwaf, waf-bypass, wfuzz | Evasione con codifica multi-livello e valutazione del bypass WAF |
...
Aggiunge OWASP ZAP e Metasploit Framework oltre al livello standard. Il provisioning richiede circa 25 minuti. Questi strumenti sono di grandi dimensioni (ZAP ~500 MiB, Metasploit ~1 GiB) e sono necessari solo per demo avanzate di scansione delle vulnerabilità.

Consultare il calcolatore dei prezzi di Azure per i costi attuali delle VM. L'istanza predefinita Standard_F16s_v2 è un'istanza ottimizzata per il calcolo, adatta alla generazione sostenuta di traffico.

:::tip
Utilizzare `terraform destroy` quando il laboratorio non è in uso per evitare addebiti continui. Consultare [Smontaggio](../08-teardown/) per la procedura.
:::
...
- **Origin Server** -- Il backend target che ospita Juice Shop, DVWA, VAmPI, httpbin e whoami. Il Traffic Generator invia traffico di attacco attraverso F5 XC per raggiungere queste applicazioni. Consultare [Integrazione](../07-integrate/) per i dettagli completi dell'architettura.

- **Demo CSD** -- L'applicazione demo Client-Side Defense sull'origin server. La suite di traffico `javascript-exploits` genera payload di script injection in stile Magecart che F5 XC Client-Side Defense rileva. Questo valida la funzionalità CSD Phase 2.
...
- **Origin Server** fornisce i target applicativi vulnerabili
...
L'operatore umano o l'assistente AI aggiunge i componenti uno alla volta. Distribuire prima l'origin server, configurare F5 XC di fronte ad esso, quindi distribuire il traffic generator puntando all'FQDN dell'HTTP load balancer di F5 XC.
