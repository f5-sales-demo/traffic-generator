...
```bash
# Eseguire la suite web-app-attacks
...
Lo stdout e lo stderr di ogni script vengono catturati in un file `.log` con il nome dello script.
...
Ogni script può essere eseguito in modo standalone senza il runner:
...
Gli script accettano il FQDN del target come primo argomento posizionale. Non leggono `config.env` direttamente -- solo il runner lo fa. Quando eseguite uno script in modo standalone, dovete passare il FQDN esplicitamente.
...
Questo genera traffico completo su tutte le funzionalità di sicurezza di F5 XC. L'esecuzione completa richiede tipicamente 45-80 minuti a seconda dei tempi di risposta del target.
...
Per le suite di lunga durata, utilizzate `nohup` o `tmux` per evitare che la disconnessione SSH termini il processo:
...
4. **Esecuzione degli script** -- Itera attraverso i file corrispondenti a `[0-9]*` nella directory della suite, ordinati per nome. Salta i file non eseguibili. Passa `TARGET_FQDN` come primo argomento. Cattura l'output in `<script-name>.log`.
