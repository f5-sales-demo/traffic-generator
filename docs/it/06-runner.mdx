...
title: Runner
description: >-
  Guida all'uso dell'orchestratore di suite runner.sh: modalità di esecuzione,
  configurazione config.env, override delle variabili d'ambiente, modalità
  dry-run, struttura della directory dei risultati, formato meta.json,
  esecuzione standalone degli script ed invocazione remota via SSH
sidebar:
  order: 6
i18n:
...
| `--dry-run` | No | Stampa ciò che verrebbe eseguito senza avviare alcuno script |

**Esempi:**

```bash
# Esegui la suite web-app-attacks
/opt/traffic-generator/suites/runner.sh web-app-attacks

# Dry-run per visualizzare in anteprima gli script di api-attacks
/opt/traffic-generator/suites/runner.sh api-attacks --dry-run

# Esegui la simulazione bot
...
Questo file viene scritto automaticamente durante il provisioning Terraform dalle variabili `target_fqdn` e `target_origin_ip`. Per cambiare il target dopo il deployment, modificate questo file direttamente sulla VM.
...
Il flag `--dry-run` stampa ogni script che verrebbe eseguito senza avviarlo:
...
Utilizzate il dry-run per verificare la struttura della suite dopo il deployment o prima di eseguire una suite su un nuovo target.
...
Lo stdout e lo stderr di ogni script vengono catturati in un file `.log` che prende il nome dallo script.

### Formato meta.json

Il file `meta.json` registra i metadati dell'esecuzione della suite:
...
| `completed` | Timestamp UTC di fine della suite |
| `status` | `running` durante l'esecuzione, `completed` al completamento |
...
## Esecuzione di script individuali
...
Gli script accettano il FQDN del target come primo argomento posizionale. Non leggono direttamente `config.env` -- solo il runner lo fa. Quando eseguite uno script in modalità standalone, dovete passare l'FQDN esplicitamente.
...
Questo genera traffico completo attraverso tutte le funzionalità di sicurezza di F5 XC. L'esecuzione completa richiede dai 45 agli 80 minuti a seconda dei tempi di risposta del target.
...
Per le suite a lunga esecuzione, utilizzate `nohup` o `tmux` per evitare che la disconnessione SSH interrompa il processo:
...
2. **Validazione della suite** -- Conferma che la directory della suite indicata esista nella directory delle suite. Elenca le suite disponibili se non trovata.
...
4. **Esecuzione degli script** -- Itera attraverso i file corrispondenti al pattern `[0-9]*` nella directory della suite, ordinati per nome. Salta i file non eseguibili. Passa `TARGET_FQDN` come primo argomento. Cattura l'output in `<script-name>.log`.

5. **Registrazione dei risultati** -- Aggiorna `meta.json` con il timestamp di completamento e i conteggi di successo/fallimento/saltati.
