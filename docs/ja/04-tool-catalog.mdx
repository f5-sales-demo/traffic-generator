...
title: ツールカタログ
description: >-
  トラフィックジェネレーター VM にインストールされた 50
  以上のセキュリティツールの完全カタログ。カテゴリ別に整理され、インストール方法、コマンド例、トラフィックスイートのマッピングを記載
sidebar:
  order: 4
i18n:
...
以下に示すすべてのツールは、cloud-init プロビジョニング時にあらかじめインストールされます。デプロイ後に手動でインストールする必要はありません。
...
GoTestWAF（Wallarm 製）は業界標準の WAF 評価フレームワークで、複数のエンコーダー（URL、Base64、JSUnicode）および配置ゾーン（ボディ、ヘッダー、URL パラメーター、クッキー）にわたってペイロードを多重化します。waf-bypass（Nemesida 製）は、ARGS、BODY、COOKIE、HEADER の各ゾーンに対して Base64、HTML エンティティ、UTF-16 エンコードのペイロードを送信します。
...
| puppeteer-extra-plugin-stealth | npm | ステルスブラウジング用の Puppeteer プラグインとして使用 | bot-simulation |

Playwright は `playwright install chromium` によって独自の Chromium バイナリをインストールします。Puppeteer は可能な場合、同じシステムの Chromium を使用します。
...
SecLists は ffuf、gobuster、feroxbuster、dirb、hydra など複数のツールが使用するワードリストを提供します。PayloadsAllTheThings は、攻撃クラス（XSS、SQLi、XXE）ごとに整理された、hex、オクタル、Unicode、HTML エンティティ、ダブル URL エンコード、JSFuck バリアントを含む精選された多重エンコード攻撃ペイロードを提供します。
...
| Metasploit Framework | インストーラースクリプト | `msfconsole -q -x "use auxiliary/scanner/http/http_version; set RHOSTS TARGET; run; exit"` | reconnaissance |

:::note
ZAP と Metasploit はディスク使用量に約 1.5 GiB を追加し、プロビジョニング時間に 5〜10 分を追加します。これらのツールがデモで特に必要な場合にのみフルティアを使用してください。
:::
...
| 方法 | 説明 |
|---|---|
| apt | `apt-get install` を使用して Ubuntu 24.04 APT リポジトリからインストール |
| ghlatest | `ghlatest.sh` と `install-release.sh` を使用して GitHub Releases から最新バイナリをダウンロード |
| uv | `uv` パッケージマネージャーを使用して Python ツールとしてインストール |
| pip | システム Python に `pip install` を使用して Python パッケージとしてインストール |
| npm | `npm install -g` を使用してグローバル Node.js パッケージとしてインストール |
| go install | `go install` を使用してソースからコンパイルし `/usr/local/bin` にインストール |
| git clone | `/opt/` ディレクトリにリポジトリをクローン |
| git clone + pip | リポジトリをクローンし Python の依存関係をインストール |
| installer script | cloud-init 時にベンダー提供のインストーラースクリプトを実行 |
