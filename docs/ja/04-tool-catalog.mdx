...
title: ツールカタログ
description: >-
  トラフィックジェネレーターVMにインストールされた50以上のセキュリティツールの完全カタログ。カテゴリ別に整理され、インストール方法、コマンド例、トラフィックスイートのマッピングを掲載
sidebar:
  order: 4
i18n:
...
以下に記載されたすべてのツールは、cloud-initプロビジョニング時にプリインストールされます。デプロイ後に手動でインストールする必要はありません。
...
GoTestWAF（Wallarm製）は、複数のエンコーダー（URL、Base64、JSUnicode）と配置ゾーン（body、header、URLパラメータ、cookie）を組み合わせてペイロードを展開する業界標準のWAF評価フレームワークです。waf-bypass（Nemesida製）は、Base64、HTML-ENTITY、UTF-16エンコーディングを使用して、ARGS、BODY、COOKIE、HEADERの各ゾーンにペイロードを送信します。
...
Playwrightは`playwright install chromium`を通じて独自のChromiumバイナリをインストールします。Puppeteerは可能な限り同じシステムChromiumを使用します。
...
SecListsは、ffuf、gobuster、feroxbuster、dirb、hydraなど複数のツールで使用されるワードリストを提供します。PayloadsAllTheThingsは、攻撃クラス（XSS、SQLi、XXE）別に整理された、hex、octal、Unicode、HTMLエンティティ、ダブルURLエンコーディング、JSFuckバリアントを含む厳選されたマルチエンコーディング攻撃ペイロードを提供します。

## フルティア限定

これらのツールは`tool_tier = "full"`の場合のみインストールされます：
...
:::note
ZAPとMetasploitは、ディスク使用量に約1.5 GiB、プロビジョニング時間に5〜10分を追加します。デモでこれらのツールが具体的に必要な場合にのみフルティアを使用してください。
:::
...
| ghlatest | `ghlatest.sh`と`install-release.sh`を使用してGitHub Releasesから最新バイナリをダウンロード |
...
| git clone + pip | リポジトリをクローンし、Pythonの依存関係をインストール |
