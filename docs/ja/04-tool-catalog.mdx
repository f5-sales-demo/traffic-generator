...
GoTestWAF（Wallarm製）は業界標準のWAF評価フレームワークで、複数のエンコーダー（URL、Base64、JSUnicode）と配置ゾーン（body、header、URLパラメータ、cookie）にわたってペイロードを展開します。waf-bypass（Nemesida製）は、Base64、HTML-ENTITY、UTF-16エンコーディングを使用して、ARGS、BODY、COOKIE、HEADERゾーンにペイロードを送信します。
...
Playwrightは`playwright install chromium`により独自のChromiumバイナリをインストールします。Puppeteerは可能な場合、同じシステムChromiumを使用します。
...
| 方法 | 説明 |
|---|---|
| apt | `apt-get install`によりUbuntu 24.04 APTリポジトリからインストール |
