...
`runner.sh` は、スイートディレクトリ内のすべての番号付きスクリプトを順番に実行し、出力をキャプチャし、結果メタデータを記録するスイートオーケストレーターです。VM上の `/opt/traffic-generator/suites/runner.sh` にインストールされます。
...
# api-attacks スクリプトをプレビューするためのドライラン
...
このファイルは、Terraformプロビジョニング時に `target_fqdn` および `target_origin_ip` 変数から自動的に書き込まれます。デプロイ後にターゲットを変更するには、VM上でこのファイルを直接編集してください。
...
`--dry-run` フラグは、実行される各スクリプトを実際に実行せずに表示します:
...
デプロイ後のスイート構造の確認や、新しいターゲットに対してスイートを実行する前の検証にドライランを使用してください。

## 結果ディレクトリ

各スイートの実行ごとにタイムスタンプ付きの結果ディレクトリが作成されます:
...
各スクリプトの stdout と stderr は、スクリプト名に基づいた `.log` ファイルにキャプチャされます。
...
| `started` | スイート開始時の UTC タイムスタンプ |
| `completed` | スイート完了時の UTC タイムスタンプ |
| `status` | 実行中は `running`、完了時は `completed` |
| `passed` | 終了コード 0 で終了したスクリプトの数 |
| `failed` | ゼロ以外の終了コードで終了したスクリプトの数 |
| `skipped` | スキップされたスクリプトの数（実行権限なしまたはドライラン） |

<Aside type="tip">
スクリプトのゼロ以外の終了コードは、必ずしもスイートが失敗したことを意味しません。多くのセキュリティツールは脆弱性を検出した際にゼロ以外で終了しますが、これは攻撃トラフィック生成時の想定された動作です。
</Aside>
...
スクリプトは最初の位置引数としてターゲット FQDN を受け取ります。スクリプトは `config.env` を直接読み取りません -- それはランナーのみが行います。スタンドアロンで実行する場合は、FQDN を明示的に渡す必要があります。
...
これにより、すべての F5 XC セキュリティ機能にわたる包括的なトラフィックが生成されます。ターゲットの応答時間に応じて、完全な実行には 45〜80 分かかることが予想されます。

## SSH 経由のリモート実行

SSH セッションを維持せずにローカルマシンからスイートを実行できます:
...
長時間実行されるスイートの場合、SSH切断によるプロセスの終了を防ぐために `nohup` または `tmux` を使用してください:
...
1. **設定の読み込み** -- ランナーは `config.env`（または `CONFIG_FILE` オーバーライド）を読み取り、環境内の `TARGET_FQDN` を確認します。
...
4. **スクリプトの実行** -- スイートディレクトリ内の `[0-9]*` にマッチするファイルを名前順にイテレートします。実行権限のないファイルはスキップします。`TARGET_FQDN` を最初の引数として渡します。出力を `<script-name>.log` にキャプチャします。

5. **結果の記録** -- 完了タイムスタンプとパス/失敗/スキップのカウントで `meta.json` を更新します。
