...
<Aside type="tip">
スクリプトのゼロ以外の終了コードは、必ずしもスイートが失敗したことを意味するわけではありません。多くのセキュリティツールは脆弱性を発見するとゼロ以外で終了しますが、これは攻撃トラフィック生成中の期待される動作です。
</Aside>
...
5. **結果の記録** -- `meta.json` に完了タイムスタンプと合格/不合格/スキップ数を更新します。
