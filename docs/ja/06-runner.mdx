...
description: >-
  ...
  設定、環境変数によるオーバーライド、ドライランモード、結果ディレクトリ構造、meta.json フォーマット、スタンドアロンスクリプト実行、リモート SSH
  呼び出し
sidebar:
  order: 6
i18n:
...
このファイルは、Terraform プロビジョニング時に `target_fqdn` および `target_origin_ip` 変数から自動的に書き込まれます。デプロイ後にターゲットを変更するには、VM 上でこのファイルを直接編集してください。

### 環境変数によるオーバーライド

`config.env` 内の任意の変数は、スイートの実行前にエクスポートすることでオーバーライドできます：
...
`--dry-run` フラグは、実行せずに各スクリプトが実行される内容を表示します：
...
ドライランを使用して、デプロイ後のスイート構造の確認や、新しいターゲットに対してスイートを実行する前の検証を行います。

## 結果ディレクトリ

各スイートの実行ごとに、タイムスタンプ付きの結果ディレクトリが作成されます：
...
`meta.json` ファイルはスイートの実行メタデータを記録します：
...
| `failed` | 非ゼロの終了コードで終了したスクリプトの数 |
| `skipped` | スキップされたスクリプトの数（実行不可またはドライラン） |

<Aside type="tip">
スクリプトの非ゼロの終了コードは、必ずしもスイートが失敗したことを意味しません。多くのセキュリティツールは脆弱性を発見すると非ゼロで終了しますが、これは攻撃トラフィック生成時の期待される動作です。
</Aside>
...
これにより、すべての F5 XC セキュリティ機能にわたる包括的なトラフィックが生成されます。ターゲットの応答時間に応じて、フル実行には 45〜80 分かかることが想定されます。

## SSH 経由のリモート実行

SSH セッションを維持せずに、ローカルマシンからスイートを実行します：
...
長時間実行されるスイートの場合、SSH の切断によりプロセスが終了するのを防ぐため、`nohup` または `tmux` を使用してください：
...
1. **設定の読み込み** -- ランナーは `config.env`（または `CONFIG_FILE` オーバーライド）を読み取り、次に環境内の `TARGET_FQDN` を確認します。
...
4. **スクリプトの実行** -- スイートディレクトリ内の `[0-9]*` にマッチするファイルを名前順に反復処理します。実行不可能なファイルはスキップします。`TARGET_FQDN` を最初の引数として渡します。出力を `<script-name>.log` にキャプチャします。

5. **結果の記録** -- 完了タイムスタンプと成功/失敗/スキップの件数で `meta.json` を更新します。
