...
GoTestWAF (da Wallarm) é um framework padrão da indústria para avaliação de WAF que multiplica payloads através de múltiplos codificadores (URL, Base64, JSUnicode) e zonas de posicionamento (body, header, URL param, cookie). waf-bypass (da Nemesida) envia payloads com codificação Base64, HTML-ENTITY e UTF-16 através de zonas ARGS, BODY, COOKIE e HEADER.
...
O Playwright instala seu próprio binário do Chromium via `playwright install chromium`. O Puppeteer usa o mesmo Chromium do sistema quando possível.
...
O SecLists fornece listas de palavras utilizadas por múltiplas ferramentas, incluindo ffuf, gobuster, feroxbuster, dirb e hydra. O PayloadsAllTheThings fornece payloads de ataque curados com múltiplas codificações, organizados por classe de ataque (XSS, SQLi, XXE) com variantes em hex, octal, Unicode, entidade HTML, codificação URL dupla e JSFuck.

## Apenas Tier Completo
...
:::note
O ZAP e o Metasploit adicionam aproximadamente 1,5 GiB ao espaço em disco e de 5 a 10 minutos ao tempo de provisionamento. Use o tier completo apenas se você precisar especificamente dessas ferramentas para uma demonstração.
:::
