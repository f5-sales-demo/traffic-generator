...
GoTestWAF (da Wallarm) é um framework padrão da indústria para avaliação de WAF que multiplica payloads através de múltiplos codificadores (URL, Base64, JSUnicode) e zonas de inserção (body, header, parâmetro de URL, cookie). waf-bypass (da Nemesida) envia payloads com codificação Base64, HTML-ENTITY e UTF-16 através das zonas ARGS, BODY, COOKIE e HEADER.
...
O SecLists fornece listas de palavras usadas por múltiplas ferramentas, incluindo ffuf, gobuster, feroxbuster, dirb e hydra. O PayloadsAllTheThings fornece payloads de ataque curados com múltiplas codificações, organizados por classe de ataque (XSS, SQLi, XXE) com variantes em hex, octal, Unicode, entidade HTML, codificação URL dupla e JSFuck.

## Somente Nível Completo
...
:::note
O ZAP e o Metasploit adicionam aproximadamente 1,5 GiB ao espaço em disco e de 5 a 10 minutos ao tempo de provisionamento. Utilize o nível completo apenas se você precisar especificamente dessas ferramentas para uma demonstração.
:::
