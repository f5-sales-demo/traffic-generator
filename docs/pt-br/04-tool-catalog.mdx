...
description: >-
  Catálogo completo com mais de 50 ferramentas de segurança instaladas na VM
  geradora de tráfego, organizadas por categoria com método de instalação,
  exemplos de comandos e mapeamentos de suíte de tráfego
sidebar:
  order: 4
i18n:
...
| Ferramenta | Método de Instalação | Exemplo de Comando | Utilizada pela Suíte |
|---|---|---|---|
...
O GoTestWAF (da Wallarm) é um framework de avaliação de WAF padrão do setor que multiplica payloads em múltiplos codificadores (URL, Base64, JSUnicode) e zonas de posicionamento (body, header, parâmetro de URL, cookie). O waf-bypass (da Nemesida) envia payloads com codificação Base64, HTML-ENTITY e UTF-16 nas zonas ARGS, BODY, COOKIE e HEADER.
...
| puppeteer-extra-plugin-stealth | npm | Utilizado como plugin do Puppeteer para navegação furtiva | bot-simulation |

O Playwright instala seu próprio binário do Chromium via `playwright install chromium`. O Puppeteer utiliza o mesmo Chromium do sistema sempre que possível.
...
| Ferramenta | Método de Instalação | Localização | Utilizada pela Suíte |
|---|---|---|---|
...
O SecLists fornece wordlists utilizadas por múltiplas ferramentas, incluindo ffuf, gobuster, feroxbuster, dirb e hydra. O PayloadsAllTheThings fornece payloads de ataque com múltiplas codificações, organizados por classe de ataque (XSS, SQLi, XXE), com variantes em hexadecimal, octal, Unicode, entidade HTML, codificação dupla de URL e JSFuck.

## Somente para o Nível Completo
...
:::note
O ZAP e o Metasploit acrescentam aproximadamente 1,5 GiB ao uso de disco e de 5 a 10 minutos ao tempo de provisionamento. Utilize o nível completo somente se você precisar especificamente dessas ferramentas para uma demonstração.
:::
...
| ghlatest | Binário mais recente baixado do GitHub Releases utilizando `ghlatest.sh` e `install-release.sh` |
| uv | Instalado como ferramenta Python utilizando o gerenciador de pacotes `uv` |
| pip | Instalado como pacote Python utilizando `pip install` no Python do sistema |
| npm | Instalado como pacote global do Node.js utilizando `npm install -g` |
...
| installer script | Script de instalação fornecido pelo fornecedor executado durante o cloud-init |
