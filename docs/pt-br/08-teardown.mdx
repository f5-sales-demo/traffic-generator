...
:::tip
Para uso recorrente de demonstração, considere automatizar o ciclo de vida completo por script: `cd terraform && terraform apply` antes de uma demonstração, execute as suítes, e então `terraform destroy` em seguida. O tier padrão é provisionado em 15-20 minutos, tornando a configuração e desmontagem no mesmo dia algo prático.
:::
