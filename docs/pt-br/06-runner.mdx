...
description: >-
  ...
  estrutura do diretório de resultados, formato meta.json, execução standalone
  de scripts e invocação remota via SSH
sidebar:
  order: 6
i18n:
...
`runner.sh` é o orquestrador de suítes que executa todos os scripts numerados em um diretório de suíte em ordem, captura a saída e registra metadados de resultados. Ele é instalado em `/opt/traffic-generator/suites/runner.sh` na VM.
...
O caminho do arquivo de configuração também pode ser substituído:
...
A flag `--dry-run` exibe cada script que seria executado sem executá-lo:
...
Cada execução de suíte cria um diretório de resultados com registro de data/hora:
...
O stdout e stderr de cada script são capturados em um arquivo `.log` nomeado conforme o script.
...
| `started` | Registro de data/hora UTC de quando a suíte iniciou |
| `completed` | Registro de data/hora UTC de quando a suíte terminou |
...
<Aside type="tip">
Um código de saída diferente de zero de um script não significa necessariamente que a suíte falhou. Muitas ferramentas de segurança retornam código diferente de zero quando encontram vulnerabilidades, o que é o comportamento esperado durante a geração de tráfego de ataque.
</Aside>

## Executando Scripts Individuais

Cada script pode ser executado de forma standalone sem o runner:
...
Os scripts aceitam o FQDN do alvo como primeiro argumento posicional. Eles não leem o `config.env` diretamente -- apenas o runner faz isso. Ao executar de forma standalone, você deve passar o FQDN explicitamente.

## Executando Todas as Suítes em Sequência

Para executar todas as suítes consecutivamente:
...
Para suítes de longa duração, use `nohup` ou `tmux` para evitar que a desconexão SSH encerre o processo:
...
1. **Carregar configuração** -- O runner lê o `config.env` (ou a substituição via `CONFIG_FILE`) e, em seguida, verifica a variável `TARGET_FQDN` no ambiente.

2. **Validar suíte** -- Confirma que o diretório da suíte nomeada existe no diretório de suítes. Lista as suítes disponíveis se não for encontrado.
...
4. **Executar scripts** -- Itera pelos arquivos correspondentes a `[0-9]*` no diretório da suíte, ordenados por nome. Ignora arquivos não executáveis. Passa `TARGET_FQDN` como primeiro argumento. Captura a saída em `<script-name>.log`.

5. **Registrar resultados** -- Atualiza o `meta.json` com o registro de data/hora de conclusão e as contagens de aprovados/reprovados/ignorados.
