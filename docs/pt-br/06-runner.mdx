---
title: Runner
description: >-
  Guia de uso do orquestrador de suítes runner.sh: modos de execução,
  configuração config.env, substituição de variáveis de ambiente, modo dry-run,
  estrutura do diretório de resultados, formato meta.json, execução standalone
  de scripts e invocação remota via SSH
sidebar:
  order: 6
i18n:
  sourceHash: c476d62976ae
  translator: machine
---

import { Steps, Aside } from '@astrojs/starlight/components';

## Visão Geral

`runner.sh` é o orquestrador de suítes que executa todos os scripts numerados em um diretório de suíte em ordem, captura a saída e registra metadados de resultados. Ele é instalado em `/opt/traffic-generator/suites/runner.sh` na VM.

## Uso

```bash
runner.sh <suite-name> [--dry-run]
```

**Argumentos:**

| Argumento | Obrigatório | Descrição |
|---|---|---|
| `suite-name` | Sim | Nome do diretório da suíte (ex.: `web-app-attacks`, `api-attacks`) |
| `--dry-run` | Não | Exibe o que seria executado sem executar nenhum script |

**Exemplos:**

```bash
# Run the web-app-attacks suite
/opt/traffic-generator/suites/runner.sh web-app-attacks

# Dry-run to preview api-attacks scripts
/opt/traffic-generator/suites/runner.sh api-attacks --dry-run

# Run bot simulation
/opt/traffic-generator/suites/runner.sh bot-simulation
```

## Configuração

### config.env

O runner lê a configuração de `/opt/traffic-generator/config.env`:

```bash
# Required: target FQDN (F5 XC load balancer domain)
TARGET_FQDN=demo.example.com

# Optional: direct origin IP for baseline testing (bypasses F5 XC)
TARGET_ORIGIN_IP=
```

Este arquivo é escrito automaticamente durante o provisionamento do Terraform a partir das variáveis `target_fqdn` e `target_origin_ip`. Para alterar o alvo após a implantação, edite este arquivo diretamente na VM.

### Substituição de Variáveis de Ambiente

Qualquer variável em `config.env` pode ser substituída exportando-a antes de executar a suíte:

```bash
# Override target FQDN for a single run
TARGET_FQDN=staging.example.com /opt/traffic-generator/suites/runner.sh web-app-attacks

# Override via export
export TARGET_FQDN=staging.example.com
/opt/traffic-generator/suites/runner.sh web-app-attacks
```

O caminho do arquivo de configuração também pode ser substituído:

```bash
CONFIG_FILE=/tmp/my-config.env /opt/traffic-generator/suites/runner.sh web-app-attacks
```

## Modo Dry-Run

A flag `--dry-run` exibe cada script que seria executado sem executá-lo:

```bash
/opt/traffic-generator/suites/runner.sh web-app-attacks --dry-run
```

Saída:

```
=== 01-sqli.sh ===
[DRY-RUN] Would execute: /opt/traffic-generator/suites/web-app-attacks/01-sqli.sh demo.example.com
=== 02-xss.sh ===
[DRY-RUN] Would execute: /opt/traffic-generator/suites/web-app-attacks/02-xss.sh demo.example.com
=== 03-command-injection.sh ===
[DRY-RUN] Would execute: /opt/traffic-generator/suites/web-app-attacks/03-command-injection.sh demo.example.com
=== 04-path-traversal.sh ===
[DRY-RUN] Would execute: /opt/traffic-generator/suites/web-app-attacks/04-path-traversal.sh demo.example.com
=== 05-nikto-scan.sh ===
[DRY-RUN] Would execute: /opt/traffic-generator/suites/web-app-attacks/05-nikto-scan.sh demo.example.com
=== 06-nuclei-scan.sh ===
[DRY-RUN] Would execute: /opt/traffic-generator/suites/web-app-attacks/06-nuclei-scan.sh demo.example.com
=== Suite Complete ===
Passed: 0 | Failed: 0 | Skipped: 6
Results: /opt/traffic-generator/results/20260425-143000-web-app-attacks
```

Use o dry-run para verificar a estrutura da suíte após a implantação ou antes de executar uma suíte contra um novo alvo.

## Diretório de Resultados

Cada execução de suíte cria um diretório de resultados com timestamp:

```
/opt/traffic-generator/results/<YYYYMMDD-HHMMSS>-<suite-name>/
```

**Estrutura:**

```
/opt/traffic-generator/results/20260425-143000-web-app-attacks/
  meta.json
  01-sqli.sh.log
  02-xss.sh.log
  03-command-injection.sh.log
  04-path-traversal.sh.log
  05-nikto-scan.sh.log
  06-nuclei-scan.sh.log
```

O stdout e stderr de cada script são capturados em um arquivo `.log` nomeado com o nome do script.

### Formato meta.json

O arquivo `meta.json` registra os metadados de execução da suíte:

```json
{
  "suite": "web-app-attacks",
  "target": "demo.example.com",
  "started": "2026-04-25T14:30:00Z",
  "completed": "2026-04-25T14:45:23Z",
  "status": "completed",
  "passed": 5,
  "failed": 1,
  "skipped": 0
}
```

| Campo | Descrição |
|---|---|
| `suite` | Nome da suíte |
| `target` | FQDN do alvo utilizado na execução |
| `started` | Timestamp UTC de quando a suíte iniciou |
| `completed` | Timestamp UTC de quando a suíte finalizou |
| `status` | `running` durante a execução, `completed` quando concluída |
| `passed` | Número de scripts que terminaram com código de saída 0 |
| `failed` | Número de scripts que terminaram com código de saída diferente de zero |
| `skipped` | Número de scripts ignorados (não executáveis ou dry-run) |

<Aside type="tip">
Um código de saída diferente de zero de um script não necessariamente significa que a suíte falhou. Muitas ferramentas de segurança retornam código diferente de zero quando encontram vulnerabilidades, o que é o comportamento esperado durante a geração de tráfego de ataque.
</Aside>

## Executando Scripts Individuais

Cada script pode ser executado de forma standalone sem o runner:

```bash
# Run a single script directly
/opt/traffic-generator/suites/web-app-attacks/01-sqli.sh demo.example.com

# Run a specific API attack
/opt/traffic-generator/suites/api-attacks/01-vampi-owasp-top10.sh demo.example.com
```

Os scripts aceitam o FQDN do alvo como o primeiro argumento posicional. Eles não leem o `config.env` diretamente -- apenas o runner faz isso. Ao executar de forma standalone, você deve passar o FQDN explicitamente.

## Executando Todas as Suítes em Sequência

Para executar todas as suítes uma após a outra:

```bash
for suite in web-app-attacks api-attacks bot-simulation reconnaissance ssl-scanning javascript-exploits traffic-generation; do
  echo "========================================="
  echo "Starting suite: ${suite}"
  echo "========================================="
  /opt/traffic-generator/suites/runner.sh "$suite"
  echo ""
done
```

Isso gera tráfego abrangente em todos os recursos de segurança do F5 XC. Espere que a execução completa leve de 45 a 80 minutos, dependendo dos tempos de resposta do alvo.

## Execução Remota via SSH

Execute suítes a partir da sua máquina local sem manter uma sessão SSH:

```bash
TGEN_IP=$(terraform output -raw public_ip)

# Run a single suite
ssh azureuser@${TGEN_IP} '/opt/traffic-generator/suites/runner.sh web-app-attacks'

# Run a suite with target override
ssh azureuser@${TGEN_IP} 'TARGET_FQDN=staging.example.com /opt/traffic-generator/suites/runner.sh api-attacks'

# Run in background (disconnection-safe)
ssh azureuser@${TGEN_IP} 'nohup /opt/traffic-generator/suites/runner.sh web-app-attacks > /tmp/web-app-attacks.log 2>&1 &'

# Check results later
ssh azureuser@${TGEN_IP} 'ls -la /opt/traffic-generator/results/ | tail -5'
ssh azureuser@${TGEN_IP} 'cat /opt/traffic-generator/results/$(ls -t /opt/traffic-generator/results/ | head -1)/meta.json'
```

Para suítes de longa duração, use `nohup` ou `tmux` para evitar que a desconexão SSH encerre o processo:

```bash
ssh azureuser@${TGEN_IP} 'tmux new-session -d -s traffic "/opt/traffic-generator/suites/runner.sh reconnaissance"'

# Reattach later to watch progress
ssh -t azureuser@${TGEN_IP} 'tmux attach-session -t traffic'
```

## Fluxo de Execução

<Steps>

1. **Carregar configuração** -- O runner lê o `config.env` (ou a substituição via `CONFIG_FILE`) e verifica a existência de `TARGET_FQDN` no ambiente.

2. **Validar suíte** -- Confirma que o diretório da suíte nomeada existe dentro do diretório de suítes. Lista as suítes disponíveis caso não seja encontrada.

3. **Criar diretório de resultados** -- Cria `/opt/traffic-generator/results/<timestamp>-<suite>/` e grava o `meta.json` inicial com `status: running`.

4. **Executar scripts** -- Itera pelos arquivos correspondentes a `[0-9]*` no diretório da suíte, ordenados por nome. Ignora arquivos não executáveis. Passa `TARGET_FQDN` como o primeiro argumento. Captura a saída em `<script-name>.log`.

5. **Registrar resultados** -- Atualiza o `meta.json` com o timestamp de conclusão e as contagens de aprovados/reprovados/ignorados.

</Steps>
