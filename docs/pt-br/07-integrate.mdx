...
description: >-
  ...
  configuração de destino, comparação de tráfego direto vs LB e implantação
  multicomponente
sidebar:
  order: 7
i18n:
...
Cada componente é implantado e configurado independentemente via Terraform. O gerador de tráfego tem como alvo o FQDN do load balancer F5 XC, não o servidor de origem diretamente.
...
2. Configure o **F5 XC HTTP load balancer** com o servidor de origem como origin pool
...
Quando `TARGET_FQDN` está definido, todos os scripts das suítes enviam tráfego para `https://<TARGET_FQDN>/...`. O load balancer F5 XC recebe as requisições, aplica as políticas de segurança e encaminha o tráfego permitido ao servidor de origem.
...
O Simulador CDN fica na frente do load balancer F5 XC, armazenando respostas em cache e adicionando cabeçalhos semelhantes aos de CDN. Para direcionar o tráfego através do CDN:
...
O gerador de tráfego suporta o envio de tráfego tanto através do F5 XC quanto diretamente para a origem. Esta comparação demonstra o valor dos recursos de segurança do F5 XC:
...
Esperado: O WAF bloqueia payloads de SQL injection, XSS e command injection. O painel de Security Events mostra requisições bloqueadas com detalhes das violações.
...
Esperado: Todos os payloads alcançam as aplicações de origem sem filtragem. Juice Shop e DVWA processam os payloads de ataque. Isso demonstra o que acontece sem a proteção do F5 XC.
