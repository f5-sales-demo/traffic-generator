...
title: Integrar
description: >-
  Guia de integração para o gerador de tráfego com servidor de origem, F5 XC
  HTTP load balancer, demo CSD e simulador CDN: diagramas de arquitetura,
  configuração de direcionamento, comparação de tráfego direto vs LB e
  implantação multicomponente
sidebar:
  order: 7
i18n:
...
O gerador de tráfego é um componente em um ambiente de demonstração multicamadas. A arquitetura completa quando todos os componentes estão implantados:
...
Cada componente é implantado e configurado de forma independente via Terraform. O gerador de tráfego direciona o FQDN do load balancer F5 XC, não o servidor de origem diretamente.

## Integração com o Servidor de Origem

O [servidor de origem](https://f5xc-salesdemos.github.io/origin-server/) fornece as aplicações backend que as suítes de ataque do gerador de tráfego direcionam:
...
| ssl-scanning | F5 XC LB (não a origem diretamente) | N/A |
...
3. Anexe as **políticas de WAF, Bot Defense, API Security e CSD** ao load balancer
...
Quando `TARGET_FQDN` está definido, todos os scripts das suítes enviam tráfego para `https://<TARGET_FQDN>/...`. O load balancer F5 XC recebe as requisições, aplica as políticas de segurança e encaminha o tráfego permitido para o servidor de origem.

## Integração com CSD Demo

A suíte `javascript-exploits` é especificamente projetada para a demonstração de Client-Side Defense no servidor de origem. Esta suíte valida a funcionalidade da Fase 2 do CSD:

**Fluxo da Fase 2:**

1. O servidor de origem hospeda a página de demonstração CSD em `/csd-demo/`
...
4. O F5 XC CSD detecta essas modificações e as reporta no painel CSD
...
Quando o Simulador CDN é implantado, a arquitetura adiciona uma camada de cache:

```
Traffic Generator -> CDN Simulator -> F5 XC HTTP LB -> Origin Server
```

O Simulador CDN fica na frente do load balancer F5 XC, fazendo cache de respostas e adicionando cabeçalhos semelhantes aos de CDN. Para direcionar o tráfego através do CDN:
...
O gerador de tráfego suporta o envio de tráfego tanto pelo F5 XC quanto diretamente para a origem. Esta comparação demonstra o valor dos recursos de segurança do F5 XC:
...
Esperado: O WAF bloqueia payloads de SQL injection, XSS e command injection. O painel de Security Events mostra as requisições bloqueadas com detalhes das violações.

### Direto para a Origem (linha de base)
...
Esperado: Todos os payloads chegam às aplicações de origem sem filtragem. Juice Shop e DVWA processam os payloads de ataque. Isso demonstra o que acontece sem a proteção do F5 XC.

### Fluxo de Demonstração Lado a Lado

Para uma demonstração convincente, execute a mesma suíte de ambas as formas:
...
4. Compare os resultados do `meta.json` da suíte: execuções diretas mostram mais "passed" (ataques bem-sucedidos), execuções pelo LB mostram mais "failed" (ataques bloqueados)
...
## Implantação Terraform Multicomponente

Ao implantar o ambiente de laboratório completo, use workspaces ou diretórios separados do Terraform para cada componente:
