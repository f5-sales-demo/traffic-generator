...
GoTestWAF（由 Wallarm 开发）是行业标准的 WAF 评估框架，它将有效载荷通过多种编码器（URL、Base64、JSUnicode）和放置区域（body、header、URL 参数、cookie）进行组合扩展。waf-bypass（由 Nemesida 开发）使用 Base64、HTML-ENTITY 和 UTF-16 编码，跨 ARGS、BODY、COOKIE 和 HEADER 区域发送有效载荷。
...
SecLists 提供多个工具使用的字典文件，包括 ffuf、gobuster、feroxbuster、dirb 和 hydra。PayloadsAllTheThings 提供按攻击类别（XSS、SQLi、XXE）组织的精选多编码攻击有效载荷，包含十六进制、八进制、Unicode、HTML 实体、双重 URL 编码和 JSFuck 变体。
...
:::note
ZAP 和 Metasploit 会增加约 1.5 GiB 的磁盘占用和 5-10 分钟的配置时间。仅在演示中确实需要这些工具时才使用完整层级。
:::
...
| pip | 使用 `pip install` 作为 Python 包安装到系统 Python 中 |
