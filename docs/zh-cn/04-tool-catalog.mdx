---
title: 工具目录
description: 安装在流量生成器虚拟机上的 50 多种安全工具完整目录，按类别组织，包含安装方式、示例命令和流量套件映射
sidebar:
  order: 4
i18n:
...
以下列出的所有工具均在 cloud-init 配置过程中预安装。部署后无需手动安装。
...
GoTestWAF（由 Wallarm 开发）是一个行业标准的 WAF 评估框架，它通过多种编码器（URL、Base64、JSUnicode）和放置区域（body、header、URL 参数、cookie）对载荷进行组合扩展。waf-bypass（由 Nemesida 开发）使用 Base64、HTML-ENTITY 和 UTF-16 编码，在 ARGS、BODY、COOKIE 和 HEADER 区域发送载荷。
...
Playwright 通过 `playwright install chromium` 安装自带的 Chromium 二进制文件。Puppeteer 尽可能使用相同的系统 Chromium。
...
SecLists 提供多个工具使用的字典文件，包括 ffuf、gobuster、feroxbuster、dirb 和 hydra。PayloadsAllTheThings 提供按攻击类别（XSS、SQLi、XXE）整理的多编码攻击载荷，包含十六进制、八进制、Unicode、HTML 实体、双重 URL 编码和 JSFuck 变体。
...
| git clone + pip | 仓库克隆后安装 Python 依赖 |
