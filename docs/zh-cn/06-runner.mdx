...
title: 运行器
description: >-
  runner.sh 套件编排器使用指南：执行模式、config.env 配置、环境变量覆盖、dry-run 模式、结果目录结构、meta.json
  格式、独立脚本执行和远程 SSH 调用
sidebar:
  order: 6
i18n:
...
此文件在 Terraform 部署期间根据 `target_fqdn` 和 `target_origin_ip` 变量自动生成。如需在部署后更改目标，请直接在虚拟机上编辑此文件。

### 环境变量覆盖

`config.env` 中的任何变量都可以通过在运行套件前导出来覆盖：
...
使用预演模式可在部署后验证套件结构，或在针对新目标运行套件之前进行检查。

## 结果目录

每次套件运行会创建一个带时间戳的结果目录：
...
每个脚本的 stdout 和 stderr 会被捕获到以脚本命名的 `.log` 文件中。
...
| `started` | 套件开始时的 UTC 时间戳 |
| `completed` | 套件完成时的 UTC 时间戳 |
| `status` | 执行期间为 `running`，完成后为 `completed` |
| `passed` | 退出码为 0 的脚本数量 |
| `failed` | 退出码为非零的脚本数量 |
| `skipped` | 被跳过的脚本数量（不可执行或处于预演模式） |

<Aside type="tip">
脚本的非零退出码并不一定意味着套件失败。许多安全工具在发现漏洞时会以非零码退出，这在攻击流量生成过程中是预期行为。
</Aside>
...
脚本接受目标 FQDN 作为第一个位置参数。它们不会直接读取 `config.env`——只有运行器才会读取。独立运行时，您必须显式传递 FQDN。
...
这将生成覆盖所有 F5 XC 安全功能的综合流量。根据目标的响应时间，完整运行预计需要 45-80 分钟。
...
对于长时间运行的套件，使用 `nohup` 或 `tmux` 可以防止 SSH 断开连接导致进程被终止：
...
2. **验证套件** -- 确认指定的套件目录存在于 suites 目录下。如果未找到，则列出可用套件。
...
5. **记录结果** -- 更新 `meta.json`，写入完成时间戳和通过/失败/跳过计数。
