---
title: 集成
description: 流量生成器与源服务器、F5 XC HTTP 负载均衡器、CSD 演示和 CDN 模拟器的集成指南：架构图、目标配置、直连与负载均衡流量对比以及多组件部署
sidebar:
  order: 7
i18n:
...
流量生成器是多层演示环境中的一个组件。所有组件部署后的完整架构如下：
...
每个组件通过 Terraform 独立部署和配置。流量生成器的目标是 F5 XC 负载均衡器的 FQDN，而非直接指向源服务器。
...
| ssl-scanning | F5 XC LB（非直接源服务器） | 不适用 |
...
当设置了 `TARGET_FQDN` 时，所有套件脚本将流量发送到 `https://<TARGET_FQDN>/...`。F5 XC 负载均衡器接收请求、应用安全策略，并将允许的流量转发到源服务器。

## CSD 演示集成

`javascript-exploits` 套件专门为源服务器上的客户端防御演示而设计。该套件验证 CSD Phase 2 功能：
...
4. F5 XC CSD 检测这些修改并在 CSD 仪表板中报告
...
部署 CDN 模拟器后，架构中增加了一个缓存层：

```
Traffic Generator -> CDN Simulator -> F5 XC HTTP LB -> Origin Server
```

CDN 模拟器位于 F5 XC 负载均衡器前面，缓存响应并添加类似 CDN 的头信息。要将流量通过 CDN 发送：
...
预期结果：所有载荷未经过滤地到达源应用程序。Juice Shop 和 DVWA 处理了攻击载荷。这展示了没有 F5 XC 保护时会发生什么。

### 并排演示流程

要进行具有说服力的演示，以两种方式运行相同的套件：

1. 直接对源服务器运行 `web-app-attacks` -- 展示攻击成功
2. 通过 F5 XC 运行 `web-app-attacks` -- 展示攻击被阻止
3. 打开 F5 XC 安全事件仪表板，显示被阻止的请求
4. 比较套件 `meta.json` 结果：直连运行显示更多 "passed"（攻击成功），LB 运行显示更多 "failed"（攻击被阻止）
...
部署完整实验环境时，为每个组件使用单独的 Terraform 工作区或目录：
