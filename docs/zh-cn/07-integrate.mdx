...
| ssl-scanning | F5 XC LB（非直连源服务器） | N/A |
...
当设置了 `TARGET_FQDN` 后，所有套件脚本会将流量发送到 `https://<TARGET_FQDN>/...`。F5 XC 负载均衡器接收请求、应用安全策略，并将允许的流量转发到源服务器。
...
2. F5 XC CSD 将其监控 JavaScript 注入页面
...
当部署了 CDN 模拟器时，架构中会增加一个缓存层：

```
Traffic Generator -> CDN Simulator -> F5 XC HTTP LB -> Origin Server
```

CDN 模拟器位于 F5 XC 负载均衡器前面，缓存响应并添加类似 CDN 的头部。要将流量通过 CDN 发送：
...
- 识别 CDN 代理头部后面的真实客户端 IP
- 对可能被 CDN 修改的请求应用 WAF 规则
- 当 CDN 修改浏览器指纹时进行 Bot Defense 分类
...
预期结果：所有载荷未经过滤地到达源应用程序。Juice Shop 和 DVWA 处理攻击载荷。这展示了没有 F5 XC 保护时会发生什么。

### 并行演示流程

为了进行有说服力的演示，以两种方式运行相同的套件：

1. 直接对源服务器运行 `web-app-attacks` -- 展示攻击成功
2. 通过 F5 XC 运行 `web-app-attacks` -- 展示攻击被阻止
3. 打开 F5 XC 安全事件仪表板查看被阻止的请求
4. 比较套件 `meta.json` 结果：直连运行显示更多 "passed"（攻击成功），负载均衡运行显示更多 "failed"（攻击被阻止）
