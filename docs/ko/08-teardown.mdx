---
title: 정리
description: 'terraform destroy로 모든 Azure 리소스 제거, az CLI로 리소스 그룹 삭제 확인, 로컬 상태 파일 정리'
sidebar:
  order: 8
i18n:
...
VM을 삭제하기 전에 트래픽 스위트 결과를 보관하려면 로컬에 복사합니다:
...
1. HTTP 로드 밸런서 삭제
2. 오리진 풀 삭제
3. 테스트를 위해 생성된 웹 앱 방화벽 (WAF) 정책, Bot 고급 방어 구성, API 보안 정책 또는 CSD 구성 삭제

이 F5 XC 리소스는 Azure 비용이 발생하지 않지만, 데모 환경이 더 이상 필요하지 않을 때 정리해야 합니다.

:::tip
반복적인 데모 사용을 위해 전체 수명 주기를 스크립트로 작성하는 것을 고려하세요: 데모 전에 `cd terraform && terraform apply`를 실행하고, 스위트를 실행한 후 `terraform destroy`를 실행합니다. 표준 티어는 15~20분 내에 프로비저닝되므로 당일 설정 및 정리가 실용적입니다.
:::
