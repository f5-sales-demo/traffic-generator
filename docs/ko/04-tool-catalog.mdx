...
title: 도구 카탈로그
description: >-
  트래픽 생성기 VM에 설치된 50개 이상의 보안 도구에 대한 전체 카탈로그로, 카테고리별로 정리되어 설치 방법, 예제 명령어 및 트래픽
  스위트 매핑을 제공합니다
sidebar:
  order: 4
i18n:
...
GoTestWAF(Wallarm 제공)는 여러 인코더(URL, Base64, JSUnicode)와 배치 영역(body, header, URL param, cookie)에 걸쳐 페이로드를 다중화하는 업계 표준 WAF 평가 프레임워크입니다. waf-bypass(Nemesida 제공)는 Base64, HTML-ENTITY, UTF-16 인코딩을 사용하여 ARGS, BODY, COOKIE, HEADER 영역에 걸쳐 페이로드를 전송합니다.
...
SecLists는 ffuf, gobuster, feroxbuster, dirb, hydra를 포함한 여러 도구에서 사용하는 워드리스트를 제공합니다. PayloadsAllTheThings는 공격 유형(XSS, SQLi, XXE)별로 정리된 큐레이션된 다중 인코딩 공격 페이로드를 제공하며, hex, octal, Unicode, HTML 엔티티, 이중 URL 인코딩, JSFuck 변형을 포함합니다.
...
:::note
ZAP과 Metasploit은 디스크 사용량에 약 1.5 GiB를 추가하고 프로비저닝 시간에 5-10분을 추가합니다. 데모에 이러한 도구가 특별히 필요한 경우에만 전체 티어를 사용하십시오.
:::
...
| installer script | cloud-init 중에 공급업체 제공 설치 스크립트 실행 |
