...
title: 도구 카탈로그
description: >-
  트래픽 생성기 VM에 설치된 50개 이상의 보안 도구 전체 카탈로그 — 카테고리별 정리, 설치 방법, 예제 명령어 및 트래픽 스위트 매핑
  포함
sidebar:
  order: 4
i18n:
...
아래 나열된 모든 도구는 cloud-init 프로비저닝 중에 사전 설치됩니다. 배포 후 수동 설치가 필요하지 않습니다.
...
GoTestWAF(Wallarm 제공)는 업계 표준 WAF 평가 프레임워크로, 여러 인코더(URL, Base64, JSUnicode)와 배치 영역(body, header, URL param, cookie)에 걸쳐 페이로드를 증식합니다. waf-bypass(Nemesida 제공)는 Base64, HTML-ENTITY, UTF-16 인코딩을 사용하여 ARGS, BODY, COOKIE, HEADER 영역에 걸쳐 페이로드를 전송합니다.
...
SecLists는 ffuf, gobuster, feroxbuster, dirb, hydra를 포함한 여러 도구에서 사용하는 워드리스트를 제공합니다. PayloadsAllTheThings는 공격 클래스(XSS, SQLi, XXE)별로 정리된 큐레이션된 다중 인코딩 공격 페이로드를 제공하며, hex, octal, Unicode, HTML 엔티티, 이중 URL 인코딩, JSFuck 변형을 포함합니다.
...
:::note
ZAP과 Metasploit은 디스크 용량을 약 1.5 GiB 추가로 사용하며 프로비저닝 시간이 5-10분 증가합니다. 데모에서 이 도구들이 구체적으로 필요한 경우에만 full 티어를 사용하세요.
:::
...
| git clone + pip | 리포지토리를 클론하고 Python 의존성 설치 |
