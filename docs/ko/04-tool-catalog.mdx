---
title: 도구 카탈로그
description: '트래픽 생성기 VM에 설치된 50개 이상의 보안 도구 전체 카탈로그, 카테고리별 정리 및 설치 방법, 예제 명령어, 트래픽 스위트 매핑 포함'
sidebar:
  order: 4
i18n:
...
아래에 나열된 모든 도구는 cloud-init 프로비저닝 중에 사전 설치됩니다. 배포 후 수동 설치가 필요하지 않습니다.

## 웹 애플리케이션 테스트

| 도구 | 설치 방법 | 예제 명령어 | 사용 스위트 |
|---|---|---|---|
...
GoTestWAF(Wallarm 제공)는 여러 인코더(URL, Base64, JSUnicode)와 배치 영역(body, header, URL param, cookie)에 걸쳐 페이로드를 곱하여 생성하는 업계 표준 WAF 평가 프레임워크입니다. waf-bypass(Nemesida 제공)는 Base64, HTML-ENTITY, UTF-16 인코딩을 사용하여 ARGS, BODY, COOKIE, HEADER 영역에 걸쳐 페이로드를 전송합니다.
...
| puppeteer-extra-plugin-stealth | npm | 스텔스 브라우징을 위한 Puppeteer 플러그인으로 사용 | bot-simulation |
...
SecLists는 ffuf, gobuster, feroxbuster, dirb, hydra를 포함한 여러 도구에서 사용하는 워드리스트를 제공합니다. PayloadsAllTheThings는 공격 클래스(XSS, SQLi, XXE)별로 정리된 큐레이션된 다중 인코딩 공격 페이로드를 제공하며, hex, octal, Unicode, HTML entity, 이중 URL 인코딩, JSFuck 변형을 포함합니다.

## Full 티어 전용

이 도구들은 `tool_tier = "full"`인 경우에만 설치됩니다:
...
:::note
ZAP과 Metasploit은 디스크 사용량에 약 1.5 GiB를 추가하고 프로비저닝 시간에 5-10분을 더합니다. 데모에 이러한 도구가 특별히 필요한 경우에만 full 티어를 사용하세요.
:::
...
| 방법 | 설명 |
|---|---|
| apt | `apt-get install`을 통해 Ubuntu 24.04 APT 리포지토리에서 설치 |
| ghlatest | `ghlatest.sh` 및 `install-release.sh`를 사용하여 GitHub Releases에서 최신 바이너리 다운로드 |
| uv | `uv` 패키지 매니저를 사용하여 Python 도구로 설치 |
...
| git clone | `/opt/` 디렉토리에 리포지토리 클론 |
| git clone + pip | 리포지토리 클론 후 Python 종속성 설치 |
| installer script | cloud-init 중에 벤더 제공 설치 스크립트 실행 |
