...
title: 사전 요구 사항
description: >-
  기여자 역할이 있는 Azure 구독, Terraform >= 1.5, SSH 키 쌍 (Ed25519 권장), subscription_id
  조회를 위한 Azure CLI, 클라우드 초기화 프로비저닝 중 도구 다운로드를 위한 아웃바운드 HTTPS
sidebar:
  order: 2
i18n:
...
`id` 값을 복사하십시오 -- 이것이 Terraform에서 요구하는 `subscription_id`입니다.
...
1. **오리진 서버**가 배포되어 접근 가능한 상태 ([origin-server 문서](https://f5-sales-demo.github.io/origin-server/en/03-deploy/) 참조)
...
:::tip
실습이 사용되지 않을 때는 `terraform destroy`를 사용하여 지속적인 비용이 발생하지 않도록 하십시오. 절차는 [Teardown](../08-teardown/)을 참조하십시오.
:::

:::note
Standard_F16s_v2 (32GiB RAM)는 스위트 실행 중 여러 도구가 동시에 실행되기 때문에 기본값으로 설정되어 있습니다. 헤드리스 Chromium을 사용하는 브라우저 자동화(Playwright, Puppeteer)가 주요 메모리 소비 요소입니다. 소형 F 시리즈 SKU는 비브라우저 스위트에서 작동하지만, 봇 시뮬레이션 실행 시 메모리가 부족할 수 있습니다.
:::
