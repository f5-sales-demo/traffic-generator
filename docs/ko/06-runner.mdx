...
description: >-
  ...
  디렉터리 구조, meta.json 형식, 독립 실행 스크립트, 원격 SSH 호출
sidebar:
  order: 6
i18n:
...
`runner.sh`는 스위트 디렉터리 내 모든 번호가 매겨진 스크립트를 순서대로 실행하고, 출력을 캡처하며, 결과 메타데이터를 기록하는 스위트 오케스트레이터입니다. VM의 `/opt/traffic-generator/suites/runner.sh`에 설치됩니다.
...
| 인수 | 필수 여부 | 설명 |
|---|---|---|
| `suite-name` | 예 | 스위트 디렉터리 이름 (예: `web-app-attacks`, `api-attacks`) |
| `--dry-run` | 아니오 | 스크립트를 실행하지 않고 실행될 항목만 출력 |
...
`config.env`의 모든 변수는 스위트 실행 전에 내보내기를 통해 오버라이드할 수 있습니다:

```bash
# 단일 실행을 위한 대상 FQDN 오버라이드
...
`--dry-run` 플래그는 각 스크립트를 실행하지 않고 실행될 내용을 출력합니다:
...
드라이런을 사용하여 배포 후 스위트 구조를 확인하거나 새로운 대상에 대해 스위트를 실행하기 전에 검증하십시오.

## 결과 디렉터리

각 스위트 실행은 타임스탬프가 포함된 결과 디렉터리를 생성합니다:
...
각 스크립트의 stdout과 stderr는 스크립트 이름을 따른 `.log` 파일에 캡처됩니다.
...
<Aside type="tip">
스크립트의 0이 아닌 종료 코드가 반드시 스위트 실패를 의미하는 것은 아닙니다. 많은 보안 도구는 취약점을 발견하면 0이 아닌 코드로 종료하며, 이는 공격 트래픽 생성 시 예상되는 동작입니다.
</Aside>
...
스크립트는 대상 FQDN을 첫 번째 위치 인수로 받습니다. 스크립트는 `config.env`를 직접 읽지 않으며, 러너만 이를 읽습니다. 독립 실행 시에는 FQDN을 명시적으로 전달해야 합니다.
...
이는 모든 F5 XC 보안 기능에 걸쳐 포괄적인 트래픽을 생성합니다. 대상의 응답 시간에 따라 전체 실행에 45-80분이 소요될 것으로 예상됩니다.
...
장시간 실행되는 스위트의 경우, SSH 연결 끊김으로 인한 프로세스 종료를 방지하기 위해 `nohup` 또는 `tmux`를 사용하십시오:
...
2. **스위트 검증** -- 지정된 스위트 디렉터리가 스위트 디렉터리 하위에 존재하는지 확인합니다. 찾을 수 없는 경우 사용 가능한 스위트를 나열합니다.

3. **결과 디렉터리 생성** -- `/opt/traffic-generator/results/<timestamp>-<suite>/`를 생성하고 `status: running`으로 초기 `meta.json`을 작성합니다.

4. **스크립트 실행** -- 스위트 디렉터리에서 `[0-9]*`에 일치하는 파일을 이름순으로 정렬하여 순회합니다. 실행 불가능한 파일은 건너뜁니다. `TARGET_FQDN`을 첫 번째 인수로 전달합니다. 출력을 `<script-name>.log`에 캡처합니다.

5. **결과 기록** -- 완료 타임스탬프와 통과/실패/건너뛰기 횟수로 `meta.json`을 업데이트합니다.
