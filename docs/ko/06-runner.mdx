...
title: Runner
description: >-
  runner.sh 스위트 오케스트레이터 사용 가이드: 실행 모드, config.env 구성, 환경 변수 재정의, 드라이런 모드, 결과
  디렉토리 구조, meta.json 형식, 독립 실행형 스크립트 실행 및 원격 SSH 호출
sidebar:
  order: 6
i18n:
...
`runner.sh`는 스위트 디렉토리 내의 번호가 매겨진 모든 스크립트를 순서대로 실행하고, 출력을 캡처하며, 결과 메타데이터를 기록하는 스위트 오케스트레이터입니다. VM의 `/opt/traffic-generator/suites/runner.sh`에 설치됩니다.
...
`config.env`의 모든 변수는 스위트 실행 전에 내보내기(export)하여 재정의할 수 있습니다:
...
드라이런을 사용하여 배포 후 또는 새 대상에 대해 스위트를 실행하기 전에 스위트 구조를 확인하십시오.

## 결과 디렉토리

각 스위트 실행은 타임스탬프가 있는 결과 디렉토리를 생성합니다:
...
각 스크립트의 stdout 및 stderr은 스크립트 이름을 딴 `.log` 파일에 캡처됩니다.
...
<Aside type="tip">
스크립트의 0이 아닌 종료 코드가 반드시 스위트 실패를 의미하지는 않습니다. 많은 보안 도구는 취약점을 발견했을 때 0이 아닌 코드로 종료되며, 이는 공격 트래픽 생성 중에 예상되는 동작입니다.
</Aside>
...
스크립트는 첫 번째 위치 인수로 대상 FQDN을 받습니다. 스크립트는 `config.env`를 직접 읽지 않으며, 러너만 이를 수행합니다. 독립적으로 실행할 때는 FQDN을 명시적으로 전달해야 합니다.
...
이렇게 하면 모든 F5 XC 보안 기능에 걸쳐 포괄적인 트래픽이 생성됩니다. 대상의 응답 시간에 따라 전체 실행에 45~80분이 소요될 수 있습니다.
...
장시간 실행되는 스위트의 경우, SSH 연결이 끊겨 프로세스가 종료되지 않도록 `nohup` 또는 `tmux`를 사용하십시오:
...
1. **구성 로드** -- 러너가 `config.env`(또는 `CONFIG_FILE` 재정의)를 읽은 후, 환경에서 `TARGET_FQDN`을 확인합니다.

2. **스위트 유효성 검사** -- 명명된 스위트 디렉토리가 스위트 디렉토리 아래에 존재하는지 확인합니다. 찾을 수 없는 경우 사용 가능한 스위트를 나열합니다.
...
5. **결과 기록** -- 완료 타임스탬프 및 통과/실패/건너뜀 수로 `meta.json`을 업데이트합니다.
