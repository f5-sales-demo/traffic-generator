...
| 인수 | 필수 여부 | 설명 |
|---|---|---|
| `suite-name` | 예 | 스위트 디렉토리의 이름 (예: `web-app-attacks`, `api-attacks`) |
...
# api-attacks 스크립트를 미리 확인하기 위한 드라이런
...
`config.env`의 모든 변수는 스위트 실행 전에 내보내기(export)하여 오버라이드할 수 있습니다:

```bash
# 단일 실행에 대한 대상 FQDN 오버라이드
...
`--dry-run` 플래그는 실제로 실행하지 않고 실행될 각 스크립트를 출력합니다:
...
배포 후 스위트 구조를 확인하거나 새로운 대상에 스위트를 실행하기 전에 드라이런을 사용하십시오.
...
각 스크립트의 stdout과 stderr는 스크립트 이름으로 된 `.log` 파일에 캡처됩니다.
...
<Aside type="tip">
스크립트의 0이 아닌 종료 코드가 반드시 스위트 실패를 의미하지는 않습니다. 많은 보안 도구는 취약점을 발견하면 0이 아닌 코드로 종료하며, 이는 공격 트래픽 생성 시 예상되는 동작입니다.
</Aside>
...
이렇게 하면 모든 F5 XC 보안 기능에 걸쳐 포괄적인 트래픽이 생성됩니다. 전체 실행은 대상의 응답 시간에 따라 45-80분 정도 소요됩니다.
...
장시간 실행되는 스위트의 경우 `nohup` 또는 `tmux`를 사용하여 SSH 연결 해제로 인한 프로세스 종료를 방지하십시오:
...
1. **구성 로드** -- 러너가 `config.env`(또는 `CONFIG_FILE` 오버라이드)를 읽은 후 환경에서 `TARGET_FQDN`을 확인합니다.

2. **스위트 검증** -- 지정된 스위트 디렉토리가 스위트 디렉토리 하위에 존재하는지 확인합니다. 찾을 수 없으면 사용 가능한 스위트 목록을 표시합니다.

3. **결과 디렉토리 생성** -- `/opt/traffic-generator/results/<timestamp>-<suite>/`를 생성하고 `status: running`으로 초기 `meta.json`을 작성합니다.

4. **스크립트 실행** -- 스위트 디렉토리에서 `[0-9]*`와 일치하는 파일을 이름순으로 정렬하여 순회합니다. 실행 불가능한 파일은 건너뜁니다. `TARGET_FQDN`을 첫 번째 인수로 전달합니다. 출력을 `<script-name>.log`에 캡처합니다.

5. **결과 기록** -- 완료 타임스탬프와 성공/실패/건너뜀 카운트로 `meta.json`을 업데이트합니다.
