...
title: 통합
description: >-
  오리진 서버, F5 XC HTTP 로드 밸런서, CSD 데모, CDN 시뮬레이터와의 트래픽 생성기 통합 가이드: 아키텍처 다이어그램, 타겟팅
  구성, 직접 연결 vs LB 트래픽 비교, 다중 컴포넌트 배포
sidebar:
  order: 7
i18n:
...
트래픽 생성기는 다중 레이어 데모 환경의 한 구성 요소입니다. 모든 구성 요소가 배포된 경우의 전체 아키텍처는 다음과 같습니다:
...
`TARGET_FQDN`이 설정되면 모든 스위트 스크립트가 `https://<TARGET_FQDN>/...`으로 트래픽을 전송합니다. F5 XC 로드 밸런서가 요청을 수신하고 보안 정책을 적용한 후 허용된 트래픽을 오리진 서버로 전달합니다.
...
4. F5 XC CSD가 이러한 수정 사항을 감지하고 CSD 대시보드에 보고합니다
...
- CDN에 의해 수정되었을 수 있는 요청에 대한 WAF 규칙 적용
...
예상 결과: WAF가 SQL 인젝션, XSS, 명령어 인젝션 페이로드를 차단합니다. Security Events 대시보드에 위반 세부 정보와 함께 차단된 요청이 표시됩니다.
...
예상 결과: 모든 페이로드가 필터링 없이 오리진 애플리케이션에 도달합니다. Juice Shop과 DVWA가 공격 페이로드를 처리합니다. 이는 F5 XC 보호 없이 어떤 일이 발생하는지를 시연합니다.

### 나란히 비교 데모 흐름

설득력 있는 데모를 위해 동일한 스위트를 양쪽 방식으로 실행합니다:
...
전체 랩 환경을 배포할 때는 각 구성 요소에 대해 별도의 Terraform 워크스페이스 또는 디렉터리를 사용합니다:
