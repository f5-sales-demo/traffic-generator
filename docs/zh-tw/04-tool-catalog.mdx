---
title: 工具目錄
description: 安裝於流量產生器 VM 上的 50+ 款安全工具完整目錄，依類別整理，附安裝方式、範例指令及流量套件對應
sidebar:
  order: 4
i18n:
...
| puppeteer-extra-plugin-stealth | npm | 作為 Puppeteer 外掛程式用於隱匿瀏覽 | bot-simulation |
...
:::note
ZAP 與 Metasploit 將為磁碟佔用增加約 1.5 GiB，並使佈建時間延長 5-10 分鐘。僅在確實需要這些工具進行示範時，才使用完整版。
:::
...
| ghlatest | 使用 `ghlatest.sh` 與 `install-release.sh` 從 GitHub Releases 下載最新二進位檔 |
| uv | 使用 `uv` 套件管理器作為 Python 工具安裝 |
| pip | 使用 `pip install` 作為 Python 套件安裝至系統 Python 環境 |
...
| installer script | 在 cloud-init 期間執行廠商提供的安裝腳本 |
