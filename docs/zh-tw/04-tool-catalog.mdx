---
title: 工具目錄
description: 完整目錄涵蓋 50+ 安裝在流量產生器 VM 上的安全工具，按類別組織，包含安裝方式、範例指令及流量套件對應
sidebar:
  order: 4
i18n:
...
以下列出的所有工具均在 cloud-init 佈建期間預先安裝。部署後無需手動安裝。
...
GoTestWAF（由 Wallarm 開發）是業界標準的 WAF 評估框架，可將攻擊負載透過多種編碼器（URL、Base64、JSUnicode）及放置區域（body、header、URL 參數、cookie）進行組合放大。waf-bypass（由 Nemesida 開發）使用 Base64、HTML-ENTITY 及 UTF-16 編碼，透過 ARGS、BODY、COOKIE 及 HEADER 區域發送攻擊負載。
...
| puppeteer-extra-plugin-stealth | npm | 作為 Puppeteer 外掛用於隱匿瀏覽 | bot-simulation |

Playwright 透過 `playwright install chromium` 安裝自有的 Chromium 二進位檔。Puppeteer 盡可能使用相同的系統 Chromium。
...
SecLists 提供多種工具使用的字典檔，包括 ffuf、gobuster、feroxbuster、dirb 及 hydra。PayloadsAllTheThings 提供依攻擊類別（XSS、SQLi、XXE）整理的精選多重編碼攻擊負載，包含十六進位、八進位、Unicode、HTML 實體、雙重 URL 編碼及 JSFuck 變體。
...
:::note
ZAP 和 Metasploit 會增加約 1.5 GiB 的磁碟使用量及 5-10 分鐘的佈建時間。僅在您確實需要這些工具進行展示時才使用完整層級。
:::
...
| npm | 使用 `npm install -g` 作為全域 Node.js 套件安裝 |
| go install | 使用 `go install` 從原始碼編譯並安裝至 `/usr/local/bin` |
| git clone | 將儲存庫克隆至 `/opt/` 目錄 |
| git clone + pip | 克隆儲存庫並安裝 Python 相依套件 |
