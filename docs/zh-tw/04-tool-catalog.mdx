---
title: 工具目錄
description: 完整目錄，涵蓋流量產生器 VM 上安裝的 50+ 安全工具，按類別整理，包含安裝方式、範例指令及流量套件對應關係
sidebar:
  order: 4
i18n:
...
GoTestWAF（由 Wallarm 開發）是業界標準的 WAF 評估框架，能跨多種編碼器（URL、Base64、JSUnicode）和放置區域（body、header、URL 參數、cookie）來乘數化酬載。waf-bypass（由 Nemesida 開發）會以 Base64、HTML-ENTITY 和 UTF-16 編碼跨 ARGS、BODY、COOKIE 和 HEADER 區域發送酬載。
...
| puppeteer-extra-plugin-stealth | npm | 作為 Puppeteer 外掛使用，用於隱匿瀏覽 | bot-simulation |

Playwright 透過 `playwright install chromium` 安裝自帶的 Chromium 二進位檔。Puppeteer 在可能的情況下使用相同的系統 Chromium。
...
SecLists 提供多個工具使用的字典檔，包括 ffuf、gobuster、feroxbuster、dirb 和 hydra。PayloadsAllTheThings 提供依攻擊類別（XSS、SQLi、XXE）整理的精選多重編碼攻擊酬載，涵蓋 hex、octal、Unicode、HTML entity、雙重 URL 編碼和 JSFuck 變體。
...
:::note
ZAP 和 Metasploit 會增加約 1.5 GiB 的磁碟佔用空間，並延長 5-10 分鐘的佈建時間。僅在展示中確實需要這些工具時才使用完整層級。
:::
