...
`runner.sh` 是套件協調器，負責按順序執行套件目錄中所有編號的腳本、擷取輸出並記錄結果中繼資料。它安裝在 VM 的 `/opt/traffic-generator/suites/runner.sh` 路徑下。
...
此檔案在 Terraform 佈建期間會從 `target_fqdn` 和 `target_origin_ip` 變數自動寫入。若要在部署後更改目標，請直接在 VM 上編輯此檔案。
...
使用乾式執行來驗證部署後的套件結構，或在針對新目標執行套件之前進行預覽。
...
每個腳本的 stdout 和 stderr 會擷取到以腳本命名的 `.log` 檔案中。
...
| `passed` | 以結束代碼 0 退出的腳本數量 |
| `failed` | 以非零結束代碼退出的腳本數量 |
| `skipped` | 被跳過的腳本數量（不可執行或乾式執行） |

<Aside type="tip">
腳本的非零結束代碼不一定表示套件失敗。許多安全工具在發現漏洞時會以非零代碼退出，這在攻擊流量生成過程中是預期的行為。
</Aside>

## 執行個別腳本

每個腳本都可以在不使用 runner 的情況下獨立執行：
...
腳本接受目標 FQDN 作為第一個位置參數。它們不會直接讀取 `config.env` —— 只有 runner 會讀取。獨立執行時，您必須明確傳遞 FQDN。
...
這會在所有 F5 XC 安全功能上產生全面的流量。根據目標的回應時間，完整執行預計需要 45-80 分鐘。
...
對於長時間執行的套件，請使用 `nohup` 或 `tmux` 來防止 SSH 斷線導致程序被終止：
...
1. **載入設定** —— Runner 讀取 `config.env`（或 `CONFIG_FILE` 覆寫），然後檢查環境中的 `TARGET_FQDN`。

2. **驗證套件** —— 確認指定的套件目錄存在於 suites 目錄下。如果未找到，則列出可用的套件。

3. **建立結果目錄** —— 建立 `/opt/traffic-generator/results/<timestamp>-<suite>/` 並寫入初始 `meta.json`，狀態為 `status: running`。

4. **執行腳本** —— 遍歷套件目錄中符合 `[0-9]*` 的檔案，按名稱排序。跳過不可執行的檔案。將 `TARGET_FQDN` 作為第一個參數傳遞。將輸出擷取到 `<script-name>.log`。

5. **記錄結果** —— 更新 `meta.json`，加入完成時間戳記及通過/失敗/跳過計數。
