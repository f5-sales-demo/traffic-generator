...
流量產生器是多層展示環境中的一個元件。當所有元件都部署完成後的完整架構如下：
...
每個元件都透過 Terraform 獨立部署和設定。流量產生器以 F5 XC 負載平衡器的 FQDN 為目標，而非直接指向來源伺服器。

## 來源伺服器整合

[來源伺服器](https://f5xc-salesdemos.github.io/origin-server/) 提供流量產生器攻擊套件所針對的後端應用程式：
...
2. 設定 **F5 XC HTTP 負載平衡器**，將來源伺服器作為來源池
...
當設定了 `TARGET_FQDN` 時，所有套件腳本會將流量發送至 `https://<TARGET_FQDN>/...`。F5 XC 負載平衡器接收請求、套用安全策略，並將允許的流量轉發至來源伺服器。

## CSD 展示整合

`javascript-exploits` 套件專門為來源伺服器上的 Client-Side Defense 展示而設計。此套件驗證 CSD Phase 2 功能：

**Phase 2 流程：**

1. 來源伺服器在 `/csd-demo/` 託管 CSD 展示頁面
2. F5 XC CSD 將其監控 JavaScript 注入頁面中
3. 流量產生器的 javascript-exploits 套件嘗試：
   - 注入模擬 Magecart 竊取器的內聯腳本
   - 修改 DOM 元素以重新導向表單提交
   - 載入未經授權的第三方 JavaScript
4. F5 XC CSD 偵測到這些修改並在 CSD 儀表板中回報
...
CDN 模擬器位於 F5 XC 負載平衡器前方，負責快取回應並添加類似 CDN 的標頭。若要將流量導向通過 CDN：
...
流量產生器支援同時透過 F5 XC 和直接發送流量至來源伺服器。此比較展示了 F5 XC 安全功能的價值：
...
預期結果：WAF 封鎖 SQL injection、XSS 和 command injection 酬載。安全事件儀表板顯示被封鎖的請求及違規詳情。
...
預期結果：所有酬載未經過濾即到達來源應用程式。Juice Shop 和 DVWA 處理攻擊酬載。這展示了沒有 F5 XC 保護時會發生的情況。

### 並排展示流程

為了進行有說服力的展示，以兩種方式執行相同的套件：

1. 直接對來源執行 `web-app-attacks` -- 展示攻擊成功
2. 透過 F5 XC 執行 `web-app-attacks` -- 展示攻擊被封鎖
3. 開啟 F5 XC 安全事件儀表板以顯示被封鎖的請求
4. 比較套件 `meta.json` 結果：直接執行顯示更多 "passed"（攻擊成功），LB 執行顯示更多 "failed"（攻擊被封鎖）
