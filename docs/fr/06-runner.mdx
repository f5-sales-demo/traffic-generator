...
description: >-
  ...
  d'exécution, configuration config.env, surcharges de variables
  d'environnement, mode dry-run, structure du répertoire des résultats, format
  meta.json, exécution autonome de scripts et invocation SSH distante
sidebar:
  order: 6
i18n:
...
`runner.sh` est l'orchestrateur de suites qui exécute tous les scripts numérotés d'un répertoire de suite dans l'ordre, capture la sortie et enregistre les métadonnées des résultats. Il est installé à `/opt/traffic-generator/suites/runner.sh` sur la VM.
...
|---|---|---|
| `suite-name` | Oui | Nom du répertoire de la suite (par ex., `web-app-attacks`, `api-attacks`) |
...
Toute variable de `config.env` peut être surchargée en l'exportant avant de lancer la suite :
...
Utilisez le dry-run pour vérifier la structure de la suite après le déploiement ou avant de lancer une suite contre une nouvelle cible.
...
Pour exécuter toutes les suites l'une après l'autre :
...
Cela génère un trafic complet couvrant toutes les fonctionnalités de sécurité F5 XC. Prévoyez 45 à 80 minutes pour l'exécution complète selon les temps de réponse de la cible.

## Exécution distante via SSH

Lancez des suites depuis votre machine locale sans maintenir une session SSH :
...
Pour les suites de longue durée, utilisez `nohup` ou `tmux` pour éviter qu'une déconnexion SSH ne tue le processus :
...
2. **Validation de la suite** -- Confirme que le répertoire de suite nommé existe sous le répertoire des suites. Liste les suites disponibles s'il n'est pas trouvé.

3. **Création du répertoire des résultats** -- Crée `/opt/traffic-generator/results/<timestamp>-<suite>/` et écrit le `meta.json` initial avec `status: running`.
