...
`runner.sh` est l'orchestrateur de suites qui exécute tous les scripts numérotés dans un répertoire de suite dans l'ordre, capture la sortie et enregistre les métadonnées des résultats. Il est installé dans `/opt/traffic-generator/suites/runner.sh` sur la VM.
...
Ce fichier est écrit automatiquement lors du provisionnement Terraform à partir des variables `target_fqdn` et `target_origin_ip`. Pour modifier la cible après le déploiement, modifiez ce fichier directement sur la VM.

### Substitution par variable d'environnement

Toute variable dans `config.env` peut être substituée en l'exportant avant l'exécution de la suite :
...
Pour les suites de longue durée, utilisez `nohup` ou `tmux` pour empêcher qu'une déconnexion SSH ne tue le processus :
