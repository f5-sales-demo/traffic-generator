...
description: >-
  Catalogue complet de plus de 50 outils de sécurité installés sur la VM de
  génération de trafic, organisés par catégorie avec la méthode d'installation,
...
Playwright installe son propre binaire Chromium via `playwright install chromium`. Puppeteer utilise le même Chromium système lorsque cela est possible.
...
SecLists fournit des listes de mots utilisées par plusieurs outils, notamment ffuf, gobuster, feroxbuster, dirb et hydra. PayloadsAllTheThings fournit des charges utiles d'attaque multi-encodées et organisées par classe d'attaque (XSS, SQLi, XXE) avec des variantes hex, octales, Unicode, entités HTML, double encodage URL et JSFuck.
...
| ghlatest | Dernier binaire téléchargé depuis les Releases GitHub à l'aide de `ghlatest.sh` et `install-release.sh` |
