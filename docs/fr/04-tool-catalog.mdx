...
GoTestWAF (par Wallarm) est un framework d'évaluation de WAF reconnu dans l'industrie qui multiplie les charges utiles à travers plusieurs encodeurs (URL, Base64, JSUnicode) et zones de placement (body, header, paramètre URL, cookie). waf-bypass (par Nemesida) envoie des charges utiles avec un encodage Base64, HTML-ENTITY et UTF-16 à travers les zones ARGS, BODY, COOKIE et HEADER.
...
SecLists fournit des listes de mots utilisées par plusieurs outils dont ffuf, gobuster, feroxbuster, dirb et hydra. PayloadsAllTheThings fournit des charges utiles d'attaque multi-encodage sélectionnées, organisées par classe d'attaque (XSS, SQLi, XXE) avec des variantes hex, octal, Unicode, entité HTML, double encodage URL et JSFuck.
...
| ghlatest | Dernier binaire téléchargé depuis les GitHub Releases à l'aide de `ghlatest.sh` et `install-release.sh` |
...
| go install | Compilé depuis les sources via `go install` et installé dans `/usr/local/bin` |
| git clone | Dépôt cloné dans le répertoire `/opt/` |
| git clone + pip | Dépôt cloné et dépendances Python installées |
| installer script | Script d'installation fourni par l'éditeur exécuté pendant cloud-init |
