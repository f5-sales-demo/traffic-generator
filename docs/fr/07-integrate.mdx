...
Chaque composant est déployé et configuré indépendamment via Terraform. Le générateur de trafic cible le FQDN du répartiteur de charge F5 XC, et non directement le serveur d'origine.
...
3. La suite javascript-exploits du générateur de trafic tente :
   - D'injecter des scripts en ligne imitant les skimmers Magecart
   - De modifier les éléments DOM pour rediriger les soumissions de formulaires
   - De charger du JavaScript tiers non autorisé
...
Le simulateur CDN se place devant le répartiteur de charge F5 XC, mettant en cache les réponses et ajoutant des en-têtes de type CDN. Pour diriger le trafic à travers le CDN :
...
- L'identification de la véritable adresse IP du client derrière les en-têtes proxy du CDN
- L'application des règles WAF aux requêtes qui peuvent avoir été modifiées par le CDN
- La classification Bot Defense lorsque le CDN modifie les empreintes numériques du navigateur

## Comparaison trafic direct vs via le LB

Le générateur de trafic prend en charge l'envoi de trafic à la fois via F5 XC et directement vers l'origine. Cette comparaison démontre la valeur des fonctionnalités de sécurité de F5 XC :
...
4. Comparez les résultats `meta.json` de la suite : les exécutions directes montrent plus de « passed » (attaques réussies), les exécutions via le LB montrent plus de « failed » (attaques bloquées)
