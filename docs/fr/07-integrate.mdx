...
| ssl-scanning | F5 XC LB (pas l'origine directement) | N/A |
...
3. La suite javascript-exploits du générateur de trafic tente de :
   - Injecter des scripts inline qui imitent les skimmers Magecart
   - Modifier les éléments DOM pour rediriger les soumissions de formulaires
...
- L'identification de la véritable IP client derrière les en-têtes proxy du CDN
- L'application des règles WAF aux requêtes qui peuvent avoir été modifiées par le CDN
- La classification Bot Defense lorsque le CDN modifie les empreintes de navigateur
...
### Directement vers l'origine (ligne de base)
...
Lors du déploiement de l'environnement de laboratoire complet, utilisez des espaces de travail ou répertoires Terraform distincts pour chaque composant :
