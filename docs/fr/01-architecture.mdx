...
title: Architecture
description: >-
  VM Azure Ubuntu 24.04 avec plus de 50 outils de sécurité pré-installés
  organisés en 19 suites de trafic, ciblant les répartiteurs de charge HTTP F5
  XC protégeant les applications serveur d'origine pour la validation WAF, Bot
  Defense, API Security et CSD
sidebar:
  order: 1
i18n:
...
Ce composant fournit une plateforme automatisée de génération de trafic qui produit du trafic d'attaque, des scans de reconnaissance, des simulations de bots et des abus d'API contre un répartiteur de charge HTTP F5 Distributed Cloud. Il représente l'« attaquant » dans une architecture de démonstration typique — la source du trafic malveillant et suspect que les fonctionnalités de sécurité F5 XC sont conçues pour détecter et bloquer.
...
Ajoute OWASP ZAP et Metasploit Framework en plus du niveau standard. Le provisionnement prend environ 25 minutes. Ces outils sont volumineux (ZAP ~500 Mio, Metasploit ~1 Gio) et ne sont nécessaires que pour les démonstrations avancées d'analyse de vulnérabilités.
...
:::tip
Utilisez `terraform destroy` lorsque le laboratoire n'est pas utilisé pour éviter les frais continus. Consultez [Démantèlement](../08-teardown/) pour la procédure.
:::
...
- **Démo CSD** -- L'application de démonstration Client-Side Defense sur le serveur d'origine. La suite de trafic `javascript-exploits` génère des charges utiles d'injection de scripts de type Magecart que F5 XC Client-Side Defense détecte. Cela valide la fonctionnalité CSD Phase 2.
