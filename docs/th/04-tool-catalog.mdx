...
description: >-
  แคตตาล็อกฉบับสมบูรณ์ของเครื่องมือรักษาความปลอดภัยกว่า 50 รายการที่ติดตั้งบน VM
  สร้างทราฟฟิก จัดหมวดหมู่ตามประเภทพร้อมวิธีการติดตั้ง คำสั่งตัวอย่าง
  และการแมปชุดทดสอบทราฟฟิก
sidebar:
  order: 4
i18n:
...
เครื่องมือทุกรายการด้านล่างนี้ได้รับการติดตั้งล่วงหน้าระหว่างการ provisioning ด้วย cloud-init ไม่จำเป็นต้องติดตั้งด้วยตนเองหลังการ deploy
...
GoTestWAF (โดย Wallarm) เป็นเฟรมเวิร์กประเมิน WAF ที่เป็นมาตรฐานอุตสาหกรรม ซึ่งทำการคูณ payload ผ่านตัวเข้ารหัสหลายรูปแบบ (URL, Base64, JSUnicode) และโซนการวาง (body, header, URL param, cookie) waf-bypass (โดย Nemesida) ส่ง payload ด้วยการเข้ารหัส Base64, HTML-ENTITY และ UTF-16 ผ่านโซน ARGS, BODY, COOKIE และ HEADER
...
| puppeteer-extra-plugin-stealth | npm | ใช้เป็นปลั๊กอิน Puppeteer สำหรับการท่องเว็บแบบซ่อนตัว | bot-simulation |

Playwright ติดตั้งไบนารี Chromium ของตัวเองผ่าน `playwright install chromium` Puppeteer ใช้ Chromium ของระบบเมื่อเป็นไปได้

## การสำรวจ Subdomain และ DNS
...
SecLists จัดเตรียม wordlist ที่ใช้โดยเครื่องมือหลายตัว รวมถึง ffuf, gobuster, feroxbuster, dirb และ hydra PayloadsAllTheThings จัดเตรียม payload การโจมตีแบบเข้ารหัสหลายรูปแบบที่คัดสรรแล้ว จัดระเบียบตามประเภทการโจมตี (XSS, SQLi, XXE) พร้อมตัวแปร hex, octal, Unicode, HTML entity, double URL encoding และ JSFuck

## ระดับ Full เท่านั้น

เครื่องมือเหล่านี้จะถูกติดตั้งเฉพาะเมื่อ `tool_tier = "full"` เท่านั้น:
...
:::note
ZAP และ Metasploit เพิ่มพื้นที่ดิสก์ประมาณ 1.5 GiB และเวลา provisioning ประมาณ 5-10 นาที ใช้ระดับ full เฉพาะเมื่อคุณต้องการเครื่องมือเหล่านี้สำหรับการสาธิตเท่านั้น
:::
...
| ghlatest | ไบนารีล่าสุดดาวน์โหลดจาก GitHub Releases โดยใช้ `ghlatest.sh` และ `install-release.sh` |
| uv | ติดตั้งเป็นเครื่องมือ Python โดยใช้ตัวจัดการแพ็กเกจ `uv` |
| pip | ติดตั้งเป็นแพ็กเกจ Python โดยใช้ `pip install` เข้าสู่ Python ของระบบ |
...
| git clone | โคลน repository ไปยังไดเรกทอรี `/opt/` |
| git clone + pip | โคลน repository และติดตั้ง dependency ของ Python |
| installer script | สคริปต์ตัวติดตั้งจากผู้ผลิตที่รันระหว่าง cloud-init |
